...
data class PetsApiClient(
  ...
  override fun deletePet(
    ...
    return Uni.createFrom().completionStage({ httpClient.sendAsync(request, BodyHandlers.ofString()) }).map({ response -> Unit })
  }
  ...
  override fun uploadPetPhoto(
    ...
    return Uni.createFrom().completionStage({ httpClient.sendAsync(request, BodyHandlers.ofString()) }).map({ response -> try {
      objectMapper.readValue(response.body(), JsonNode::class.java)
    } catch (e: Exception) {
      throw RuntimeException(e)
    }  })
  }
