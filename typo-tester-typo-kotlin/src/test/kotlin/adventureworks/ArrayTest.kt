...
class ArrayTest {
    ...
    fun canQueryPgtestnullWithDSL() {
        WithConnection.run { c ->
            ...
            assertJsonEquals(row.char, pgtestnullRepo.select().where { it.char_().isEqual(row.char) }.toList(c).first().char)
    ...
    fun canQueryPgtestWithDSL() {
        WithConnection.run { c ->
            ...
            pgtestRepo.update().setValue({ it.char_() }, row.char).where { it.uuid().isEqual(row.uuid) }.execute(c)
