 ...
 */
public record ForeignKey<Fields2, Row2>(
    String name,
    List<ColumnPair<?, Fields2>> columnPairs
) {
    
    public <T> ForeignKey<Fields2, Row2> withColumnPair(
            SqlExpr.FieldLike<T, ?> field,
            Function<Fields2, SqlExpr.FieldLike<T, Row2>> thatField) {
    ...
    public record ColumnPair<T, Fields2>(
        SqlExpr<T> thisField,
        Function<Fields2, SqlExpr<T>> thatField
    ) {}
}