...
class PaymentsRepoImpl() : PaymentsRepo {
  ...
  override fun deleteById(
    ...
    c: Connection
  ): Boolean = interpolate(Fragment.lit("delete from `payments` where `payment_id` = "), Fragment.encode(PaymentsId.pgType, paymentId), Fragment.lit("")).update().runUnchecked(c) > 0
  ...
  override fun insert(
    ...
    values.add(interpolate(Fragment.encode(KotlinDbTypes.MariaTypes.numeric, unsaved.amount), Fragment.lit("")))
    ...
    val q: Fragment = interpolate(Fragment.lit("insert into `payments`("), Fragment.comma(columns), Fragment.lit(")\nvalues ("), Fragment.comma(values), Fragment.lit(")\nreturning `payment_id`, `order_id`, `method_id`, `transaction_id`, `amount`, `currency_code`, `status`, `processor_response`, `error_message`, `ip_address`, `created_at`, `processed_at`\n"))
  ...
  override fun selectAll(c: Connection): List<PaymentsRow> = interpolate(Fragment.lit("select `payment_id`, `order_id`, `method_id`, `transaction_id`, `amount`, `currency_code`, `status`, `processor_response`, `error_message`, `ip_address`, `created_at`, `processed_at`\nfrom `payments`\n")).query(PaymentsRow._rowParser.all()).runUnchecked(c)

  override fun selectById(
    paymentId: PaymentsId,
    c: Connection
  ): PaymentsRow? = interpolate(Fragment.lit("select `payment_id`, `order_id`, `method_id`, `transaction_id`, `amount`, `currency_code`, `status`, `processor_response`, `error_message`, `ip_address`, `created_at`, `processed_at`\nfrom `payments`\nwhere `payment_id` = "), Fragment.encode(PaymentsId.pgType, paymentId), Fragment.lit("")).query(PaymentsRow._rowParser.first()).runUnchecked(c)
  ...
  override fun upsertBatch(
    ...
    c: Connection
  ): List<PaymentsRow> = interpolate(Fragment.lit("INSERT INTO `payments`(`payment_id`, `order_id`, `method_id`, `transaction_id`, `amount`, `currency_code`, `status`, `processor_response`, `error_message`, `ip_address`, `created_at`, `processed_at`)\nVALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\nON DUPLICATE KEY UPDATE `order_id` = VALUES(`order_id`),\n`method_id` = VALUES(`method_id`),\n`transaction_id` = VALUES(`transaction_id`),\n`amount` = VALUES(`amount`),\n`currency_code` = VALUES(`currency_code`),\n`status` = VALUES(`status`),\n`processor_response` = VALUES(`processor_response`),\n`error_message` = VALUES(`error_message`),\n`ip_address` = VALUES(`ip_address`),\n`created_at` = VALUES(`created_at`),\n`processed_at` = VALUES(`processed_at`)\nRETURNING `payment_id`, `order_id`, `method_id`, `transaction_id`, `amount`, `currency_code`, `status`, `processor_response`, `error_message`, `ip_address`, `created_at`, `processed_at`"))
