...
class VDailySalesViewRepoImpl() : VDailySalesViewRepo {
  ...
  override fun selectAll(c: Connection): List<VDailySalesViewRow> = interpolate(Fragment.lit("select `order_date`, `order_count`, `unique_customers`, `items_sold`, `gross_sales`, `total_discounts`, `total_shipping`, `total_tax`, `net_sales`, `avg_order_value`\nfrom `v_daily_sales`\n")).query(VDailySalesViewRow._rowParser.all()).runUnchecked(c)
}