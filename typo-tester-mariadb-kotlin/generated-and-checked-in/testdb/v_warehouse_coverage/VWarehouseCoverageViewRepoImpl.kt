...
class VWarehouseCoverageViewRepoImpl() : VWarehouseCoverageViewRepo {
  ...
  override fun selectAll(c: Connection): List<VWarehouseCoverageViewRow> = interpolate(Fragment.lit("select `warehouse_id`, `code`, `name`, `address`, `location_wkt`, `service_area_wkt`, `timezone`, `is_active`, `products_stocked`, `total_inventory`\nfrom `v_warehouse_coverage`\n")).query(VWarehouseCoverageViewRow._rowParser.all()).runUnchecked(c)
}