...
class CustomerStatusRepoImpl() : CustomerStatusRepo {
  ...
  override fun deleteById(
    ...
    c: Connection
  ): Boolean = interpolate(Fragment.lit("delete from `customer_status` where `status_code` = "), Fragment.encode(CustomerStatusId.pgType, statusCode), Fragment.lit("")).update().runUnchecked(c) > 0
  ...
  override fun insert(
    ...
    val q: Fragment = interpolate(Fragment.lit("insert into `customer_status`("), Fragment.comma(columns), Fragment.lit(")\nvalues ("), Fragment.comma(values), Fragment.lit(")\nreturning `status_code`, `description`, `is_active`\n"))
  ...
  override fun selectAll(c: Connection): List<CustomerStatusRow> = interpolate(Fragment.lit("select `status_code`, `description`, `is_active`\nfrom `customer_status`\n")).query(CustomerStatusRow._rowParser.all()).runUnchecked(c)

  override fun selectById(
    statusCode: CustomerStatusId,
    c: Connection
  ): CustomerStatusRow? = interpolate(Fragment.lit("select `status_code`, `description`, `is_active`\nfrom `customer_status`\nwhere `status_code` = "), Fragment.encode(CustomerStatusId.pgType, statusCode), Fragment.lit("")).query(CustomerStatusRow._rowParser.first()).runUnchecked(c)
  ...
  override fun upsertBatch(
    ...
    c: Connection
  ): List<CustomerStatusRow> = interpolate(Fragment.lit("INSERT INTO `customer_status`(`status_code`, `description`, `is_active`)\nVALUES (?, ?, ?)\nON DUPLICATE KEY UPDATE `description` = VALUES(`description`),\n`is_active` = VALUES(`is_active`)\nRETURNING `status_code`, `description`, `is_active`"))
