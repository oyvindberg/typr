...
class TitledpersonRepoImpl extends TitledpersonRepo {
  ...
  sql"""insert into "public"."titledperson"("title_short", "title", "name")
    values (${Fragment.encode(TitleDomainId.dbType, unsaved.titleShort)}::text, ${Fragment.encode(TitleId.dbType, unsaved.title)}, ${Fragment.encode(PgTypes.text, unsaved.name)})
