...
object EmployeedepartmenthistoryRowUnsaved {
  given pgText: PgText[EmployeedepartmenthistoryRowUnsaved] = PgText.instance((row, sb) => { BusinessentityId.dbType.text.unsafeEncode(row.businessentityid, sb); sb.append(PgText.DELIMETER); DepartmentId.dbType.text.unsafeEncode(row.departmentid, sb); sb.append(PgText.DELIMETER); ShiftId.dbType.text.unsafeEncode(row.shiftid, sb); sb.append(PgText.DELIMETER); PgTypes.date.text.unsafeEncode(row.startdate, sb); sb.append(PgText.DELIMETER); PgTypes.date.nullable.text.unsafeEncode(row.enddate, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.timestamp.text).unsafeEncode(row.modifieddate, sb) })
}