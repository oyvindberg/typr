...
object AddressId {
  ...
  given dbType: PgType[AddressId] = PgTypes.int4.bimap(AddressId.apply, _.value)

  given dbTypeArray: PgType[Array[AddressId]] = PgTypes.int4Array.bimap(xs => xs.map(AddressId.apply), xs => xs.map(_.value))
}