...
object DepartmentId {
  ...
  given dbType: PgType[DepartmentId] = PgTypes.int4.bimap(DepartmentId.apply, _.value)

  given dbTypeArray: PgType[Array[DepartmentId]] = PgTypes.int4Array.bimap(xs => xs.map(DepartmentId.apply), xs => xs.map(_.value))
}