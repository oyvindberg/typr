    ...
    fun testOpenEnumPatternMatching() {
        WithConnection.run { c ->
            ...
            val found = titledPersonRepo.select()
                .joinFk({ tp -> tp.fkTitle() }, titleRepo.select().where { t -> t.code().among(TitleId.Known.dr) })
                .joinFk({ tp_t -> tp_t._1().fkTitleDomain() }, titleDomainRepo.select().where { td -> td.code().among(TitleDomainId.Known.dr) })
