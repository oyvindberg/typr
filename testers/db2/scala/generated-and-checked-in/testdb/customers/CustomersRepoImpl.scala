...
class CustomersRepoImpl extends CustomersRepo {
  ...
  sql"""SELECT "CUSTOMER_ID", "NAME", "EMAIL", "CREATED_AT" FROM FINAL TABLE (INSERT INTO "CUSTOMERS"("CUSTOMER_ID", "NAME", "EMAIL", "CREATED_AT")
    VALUES (${Fragment.encode(CustomersId.pgType, unsaved.customerId)}, ${Fragment.encode(Db2Types.varchar, unsaved.name)}, ${Fragment.encode(Db2Types.varchar, unsaved.email)}, ${Fragment.encode(Db2Types.timestamp.nullable, unsaved.createdAt)}))
    """
  ...
  override def insert(unsaved: CustomersRowUnsaved)(using c: Connection): CustomersRow = {
    ...
    unsaved.customerId.visit(
      {  },
      value => { columns.addOne(Fragment.lit(""""CUSTOMER_ID"""")): @scala.annotation.nowarn; values.addOne(sql"${Fragment.encode(CustomersId.pgType, value)}"): @scala.annotation.nowarn }
    );
  ...
  override def upsert(unsaved: CustomersRow)(using c: Connection): Unit = {
    ...
    USING (VALUES (${Fragment.encode(CustomersId.pgType, unsaved.customerId)}, ${Fragment.encode(Db2Types.varchar, unsaved.name)}, ${Fragment.encode(Db2Types.varchar, unsaved.email)}, ${Fragment.encode(Db2Types.timestamp.nullable, unsaved.createdAt)})) AS s("CUSTOMER_ID", "NAME", "EMAIL", "CREATED_AT")
    ON t."CUSTOMER_ID" = s."CUSTOMER_ID"
    WHEN MATCHED THEN UPDATE SET "NAME" = s."NAME",
    "EMAIL" = s."EMAIL",
    "CREATED_AT" = s."CREATED_AT"
    WHEN NOT MATCHED THEN INSERT ("CUSTOMER_ID", "NAME", "EMAIL", "CREATED_AT") VALUES (${Fragment.encode(CustomersId.pgType, unsaved.customerId)}, ${Fragment.encode(Db2Types.varchar, unsaved.name)}, ${Fragment.encode(Db2Types.varchar, unsaved.email)}, ${Fragment.encode(Db2Types.timestamp.nullable, unsaved.createdAt)})"""
      .update()
