...
class OrdersRepoImpl extends OrdersRepo {
  ...
  sql"""SELECT "ORDER_ID", "CUSTOMER_ID", "ORDER_DATE", "TOTAL_AMOUNT", "STATUS" FROM FINAL TABLE (INSERT INTO "ORDERS"("CUSTOMER_ID", "ORDER_DATE", "TOTAL_AMOUNT", "STATUS")
    VALUES (${Fragment.encode(CustomersId.pgType, unsaved.customerId)}, ${Fragment.encode(Db2Types.date, unsaved.orderDate)}, ${Fragment.encode(ScalaDbTypes.Db2Types.decimal.nullable, unsaved.totalAmount)}, ${Fragment.encode(Db2Types.varchar.nullable, unsaved.status)}))
    """
  ...
  override def upsert(unsaved: OrdersRow)(using c: Connection): Unit = {
    ...
    USING (VALUES (${Fragment.encode(CustomersId.pgType, unsaved.customerId)}, ${Fragment.encode(Db2Types.date, unsaved.orderDate)}, ${Fragment.encode(ScalaDbTypes.Db2Types.decimal.nullable, unsaved.totalAmount)}, ${Fragment.encode(Db2Types.varchar.nullable, unsaved.status)})) AS s("CUSTOMER_ID", "ORDER_DATE", "TOTAL_AMOUNT", "STATUS")
    ON t."ORDER_ID" = s."ORDER_ID"
    WHEN MATCHED THEN UPDATE SET "CUSTOMER_ID" = s."CUSTOMER_ID",
    "ORDER_DATE" = s."ORDER_DATE",
    "TOTAL_AMOUNT" = s."TOTAL_AMOUNT",
    "STATUS" = s."STATUS"
    WHEN NOT MATCHED THEN INSERT ("CUSTOMER_ID", "ORDER_DATE", "TOTAL_AMOUNT", "STATUS") VALUES (${Fragment.encode(CustomersId.pgType, unsaved.customerId)}, ${Fragment.encode(Db2Types.date, unsaved.orderDate)}, ${Fragment.encode(ScalaDbTypes.Db2Types.decimal.nullable, unsaved.totalAmount)}, ${Fragment.encode(Db2Types.varchar.nullable, unsaved.status)})"""
      .update()
