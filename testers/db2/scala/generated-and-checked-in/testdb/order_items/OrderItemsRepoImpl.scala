...
class OrderItemsRepoImpl extends OrderItemsRepo {
  ...
  override def deleteById(compositeId: OrderItemsId)(using c: Connection): Boolean = sql"""delete from "ORDER_ITEMS" where "ORDER_ID" = ${Fragment.encode(OrdersId.pgType, compositeId.orderId)} AND "ITEM_NUMBER" = ${Fragment.encode(ScalaDbTypes.Db2Types.integer, compositeId.itemNumber)}""".update().runUnchecked(c) > 0

  override def deleteByIds(compositeIds: Array[OrderItemsId])(using c: Connection): Int = {
    val fragments: ListBuffer[Fragment] = ListBuffer()
    compositeIds.foreach { id => fragments.addOne(Fragment.interpolate(Fragment.lit("("), Fragment.encode(OrdersId.pgType, id.orderId), Fragment.lit(", "), Fragment.encode(ScalaDbTypes.Db2Types.integer, id.itemNumber), Fragment.lit(")"))): @scala.annotation.nowarn }
  ...
  sql"""SELECT "ORDER_ID", "ITEM_NUMBER", "PRODUCT_NAME", "QUANTITY", "UNIT_PRICE" FROM FINAL TABLE (INSERT INTO "ORDER_ITEMS"("ORDER_ID", "ITEM_NUMBER", "PRODUCT_NAME", "QUANTITY", "UNIT_PRICE")
    VALUES (${Fragment.encode(OrdersId.pgType, unsaved.orderId)}, ${Fragment.encode(ScalaDbTypes.Db2Types.integer, unsaved.itemNumber)}, ${Fragment.encode(Db2Types.varchar, unsaved.productName)}, ${Fragment.encode(ScalaDbTypes.Db2Types.integer, unsaved.quantity)}, ${Fragment.encode(ScalaDbTypes.Db2Types.decimal, unsaved.unitPrice)}))
    """
  ...
  override def selectById(compositeId: OrderItemsId)(using c: Connection): Option[OrderItemsRow] = {
    sql"""select "ORDER_ID", "ITEM_NUMBER", "PRODUCT_NAME", "QUANTITY", "UNIT_PRICE"
    from "ORDER_ITEMS"
    where "ORDER_ID" = ${Fragment.encode(OrdersId.pgType, compositeId.orderId)} AND "ITEM_NUMBER" = ${Fragment.encode(ScalaDbTypes.Db2Types.integer, compositeId.itemNumber)}""".query(OrderItemsRow.`_rowParser`.first()).runUnchecked(c)
  }
