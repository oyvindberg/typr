...
class Db2testUniqueRepoImpl extends Db2testUniqueRepo {
  ...
  override def deleteById(id: Db2testUniqueId)(using c: Connection): Boolean = sql"""delete from "DB2TEST_UNIQUE" where "ID" = ${Fragment.encode(Db2testUniqueId.pgType, id)}""".update().runUnchecked(c) > 0

  override def deleteByIds(ids: Array[Db2testUniqueId])(using c: Connection): Int = {
    val fragments: ListBuffer[Fragment] = ListBuffer()
    ids.foreach { id => fragments.addOne(Fragment.encode(Db2testUniqueId.pgType, id)): @scala.annotation.nowarn }
  ...
  override def selectById(id: Db2testUniqueId)(using c: Connection): Option[Db2testUniqueRow] = {
    sql"""select "ID", "EMAIL", "CODE", "CATEGORY"
    from "DB2TEST_UNIQUE"
    where "ID" = ${Fragment.encode(Db2testUniqueId.pgType, id)}""".query(Db2testUniqueRow.`_rowParser`.first()).runUnchecked(c)
  }
  ...
  override def update(row: Db2testUniqueRow)(using c: Connection): Boolean = {
    ...
    where "ID" = ${Fragment.encode(Db2testUniqueId.pgType, id)}""".update().runUnchecked(c) > 0
  }

  override def upsert(unsaved: Db2testUniqueRow)(using c: Connection): Unit = {
    sql"""MERGE INTO "DB2TEST_UNIQUE" AS t
    USING (VALUES (${Fragment.encode(Db2Types.varchar, unsaved.email)}, ${Fragment.encode(Db2Types.varchar, unsaved.code)}, ${Fragment.encode(Db2Types.varchar, unsaved.category)})) AS s("EMAIL", "CODE", "CATEGORY")
    ON t."ID" = s."ID"
    WHEN MATCHED THEN UPDATE SET "EMAIL" = s."EMAIL",
    "CODE" = s."CODE",
    "CATEGORY" = s."CATEGORY"
    WHEN NOT MATCHED THEN INSERT ("EMAIL", "CODE", "CATEGORY") VALUES (${Fragment.encode(Db2Types.varchar, unsaved.email)}, ${Fragment.encode(Db2Types.varchar, unsaved.code)}, ${Fragment.encode(Db2Types.varchar, unsaved.category)})"""
      .update()
