...
class DistinctTypeTestRepoImpl extends DistinctTypeTestRepo {
  ...
  override def deleteById(id: DistinctTypeTestId)(using c: Connection): Boolean = sql"""delete from "DISTINCT_TYPE_TEST" where "ID" = ${Fragment.encode(DistinctTypeTestId.pgType, id)}""".update().runUnchecked(c) > 0

  override def deleteByIds(ids: Array[DistinctTypeTestId])(using c: Connection): Int = {
    val fragments: ListBuffer[Fragment] = ListBuffer()
    ids.foreach { id => fragments.addOne(Fragment.encode(DistinctTypeTestId.pgType, id)): @scala.annotation.nowarn }
  ...
  sql"""SELECT "ID", "EMAIL", "BALANCE" FROM FINAL TABLE (INSERT INTO "DISTINCT_TYPE_TEST"("EMAIL", "BALANCE")
    VALUES (${Fragment.encode(EmailAddress.pgType, unsaved.email)}, ${Fragment.encode(MoneyAmount.pgType.nullable, unsaved.balance)}))
    """
  ...
  override def insert(unsaved: DistinctTypeTestRowUnsaved)(using c: Connection): DistinctTypeTestRow = {
    ...
    values.addOne(sql"${Fragment.encode(EmailAddress.pgType, unsaved.email)}"): @scala.annotation.nowarn
    columns.addOne(Fragment.lit(""""BALANCE"""")): @scala.annotation.nowarn
    values.addOne(sql"${Fragment.encode(MoneyAmount.pgType.nullable, unsaved.balance)}"): @scala.annotation.nowarn
    val q: Fragment = {
  ...
  override def selectById(id: DistinctTypeTestId)(using c: Connection): Option[DistinctTypeTestRow] = {
    sql"""select "ID", "EMAIL", "BALANCE"
    from "DISTINCT_TYPE_TEST"
    where "ID" = ${Fragment.encode(DistinctTypeTestId.pgType, id)}""".query(DistinctTypeTestRow.`_rowParser`.first()).runUnchecked(c)
  }
  ...
  override def update(row: DistinctTypeTestRow)(using c: Connection): Boolean = {
    ...
    set "EMAIL" = ${Fragment.encode(EmailAddress.pgType, row.email)},
    "BALANCE" = ${Fragment.encode(MoneyAmount.pgType.nullable, row.balance)}
    where "ID" = ${Fragment.encode(DistinctTypeTestId.pgType, id)}""".update().runUnchecked(c) > 0
  }

  override def upsert(unsaved: DistinctTypeTestRow)(using c: Connection): Unit = {
    sql"""MERGE INTO "DISTINCT_TYPE_TEST" AS t
    USING (VALUES (${Fragment.encode(EmailAddress.pgType, unsaved.email)}, ${Fragment.encode(MoneyAmount.pgType.nullable, unsaved.balance)})) AS s("EMAIL", "BALANCE")
    ON t."ID" = s."ID"
    WHEN MATCHED THEN UPDATE SET "EMAIL" = s."EMAIL",
    "BALANCE" = s."BALANCE"
    WHEN NOT MATCHED THEN INSERT ("EMAIL", "BALANCE") VALUES (${Fragment.encode(EmailAddress.pgType, unsaved.email)}, ${Fragment.encode(MoneyAmount.pgType.nullable, unsaved.balance)})"""
      .update()
