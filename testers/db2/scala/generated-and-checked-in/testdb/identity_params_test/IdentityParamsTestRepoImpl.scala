...
class IdentityParamsTestRepoImpl extends IdentityParamsTestRepo {
  ...
  override def deleteById(id: IdentityParamsTestId)(using c: Connection): Boolean = sql"""delete from "IDENTITY_PARAMS_TEST" where "ID" = ${Fragment.encode(IdentityParamsTestId.pgType, id)}""".update().runUnchecked(c) > 0

  override def deleteByIds(ids: Array[IdentityParamsTestId])(using c: Connection): Int = {
    val fragments: ListBuffer[Fragment] = ListBuffer()
    ids.foreach { id => fragments.addOne(Fragment.encode(IdentityParamsTestId.pgType, id)): @scala.annotation.nowarn }
  ...
  override def selectById(id: IdentityParamsTestId)(using c: Connection): Option[IdentityParamsTestRow] = {
    sql"""select "ID", "NAME"
    from "IDENTITY_PARAMS_TEST"
    where "ID" = ${Fragment.encode(IdentityParamsTestId.pgType, id)}""".query(IdentityParamsTestRow.`_rowParser`.first()).runUnchecked(c)
  }
  ...
  override def update(row: IdentityParamsTestRow)(using c: Connection): Boolean = {
    ...
    where "ID" = ${Fragment.encode(IdentityParamsTestId.pgType, id)}""".update().runUnchecked(c) > 0
  }
