/**
 * File has been automatically generated by `typo`.
 *
 * <p>IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package testdb.db2test_identity_always;

import dev.typr.foundations.dsl.DeleteBuilder;
import dev.typr.foundations.dsl.DeleteBuilderMock;
import dev.typr.foundations.dsl.DeleteParams;
import dev.typr.foundations.dsl.SelectBuilder;
import dev.typr.foundations.dsl.SelectBuilderMock;
import dev.typr.foundations.dsl.SelectParams;
import dev.typr.foundations.dsl.UpdateBuilder;
import dev.typr.foundations.dsl.UpdateBuilderMock;
import dev.typr.foundations.dsl.UpdateParams;
import java.sql.Connection;
import java.util.ArrayList;
import java.util.HashMap;
import java.util.Iterator;
import java.util.List;
import java.util.Map;
import java.util.Optional;
import java.util.function.Function;
import java.util.stream.Collectors;

public record Db2testIdentityAlwaysRepoMock(
    java.util.function.Function<Db2testIdentityAlwaysRowUnsaved, Db2testIdentityAlwaysRow> toRow,
    HashMap<Db2testIdentityAlwaysId, Db2testIdentityAlwaysRow> map)
    implements Db2testIdentityAlwaysRepo {
  public Db2testIdentityAlwaysRepoMock(
      java.util.function.Function<Db2testIdentityAlwaysRowUnsaved, Db2testIdentityAlwaysRow>
          toRow) {
    this(toRow, new HashMap<Db2testIdentityAlwaysId, Db2testIdentityAlwaysRow>());
  }
  ;

  public Db2testIdentityAlwaysRepoMock withToRow(
      java.util.function.Function<Db2testIdentityAlwaysRowUnsaved, Db2testIdentityAlwaysRow>
          toRow) {
    return new Db2testIdentityAlwaysRepoMock(toRow, map);
  }
  ;

  public Db2testIdentityAlwaysRepoMock withMap(
      HashMap<Db2testIdentityAlwaysId, Db2testIdentityAlwaysRow> map) {
    return new Db2testIdentityAlwaysRepoMock(toRow, map);
  }
  ;

  @Override
  public DeleteBuilder<Db2testIdentityAlwaysFields, Db2testIdentityAlwaysRow> delete() {
    return new DeleteBuilderMock<>(
        Db2testIdentityAlwaysFields.structure,
        () -> new ArrayList<>(map.values()),
        DeleteParams.empty(),
        row -> row.id(),
        id -> map.remove(id));
  }
  ;

  @Override
  public Boolean deleteById(Db2testIdentityAlwaysId id, Connection c) {
    return Optional.ofNullable(map.remove(id)).isPresent();
  }
  ;

  @Override
  public Integer deleteByIds(Db2testIdentityAlwaysId[] ids, Connection c) {
    var count = 0;
    for (var id : ids) {
      if (Optional.ofNullable(map.remove(id)).isPresent()) {
        count = count + 1;
        ;
      }
      ;
    }
    ;
    return count;
  }
  ;

  @Override
  public Db2testIdentityAlwaysRow insert(Db2testIdentityAlwaysRow unsaved, Connection c) {
    if (map.containsKey(unsaved.id())) {
      throw new RuntimeException("id " + unsaved.id() + " already exists");
    }
    ;
    map.put(unsaved.id(), unsaved);
    return unsaved;
  }
  ;

  @Override
  public Db2testIdentityAlwaysRow insert(Db2testIdentityAlwaysRowUnsaved unsaved, Connection c) {
    return insert(toRow.apply(unsaved), c);
  }
  ;

  @Override
  public SelectBuilder<Db2testIdentityAlwaysFields, Db2testIdentityAlwaysRow> select() {
    return new SelectBuilderMock<>(
        Db2testIdentityAlwaysFields.structure,
        () -> new ArrayList<>(map.values()),
        SelectParams.empty());
  }
  ;

  @Override
  public List<Db2testIdentityAlwaysRow> selectAll(Connection c) {
    return new ArrayList<>(map.values());
  }
  ;

  @Override
  public Optional<Db2testIdentityAlwaysRow> selectById(Db2testIdentityAlwaysId id, Connection c) {
    return Optional.ofNullable(map.get(id));
  }
  ;

  @Override
  public List<Db2testIdentityAlwaysRow> selectByIds(Db2testIdentityAlwaysId[] ids, Connection c) {
    var result = new ArrayList<Db2testIdentityAlwaysRow>();
    for (var id : ids) {
      var opt = Optional.ofNullable(map.get(id));
      if (opt.isPresent()) {
        result.add(opt.get());
      }
      ;
    }
    ;
    return result;
  }
  ;

  @Override
  public Map<Db2testIdentityAlwaysId, Db2testIdentityAlwaysRow> selectByIdsTracked(
      Db2testIdentityAlwaysId[] ids, Connection c) {
    return selectByIds(ids, c).stream()
        .collect(Collectors.toMap((Db2testIdentityAlwaysRow row) -> row.id(), Function.identity()));
  }
  ;

  @Override
  public UpdateBuilder<Db2testIdentityAlwaysFields, Db2testIdentityAlwaysRow> update() {
    return new UpdateBuilderMock<>(
        Db2testIdentityAlwaysFields.structure,
        () -> new ArrayList<>(map.values()),
        UpdateParams.empty(),
        row -> row);
  }
  ;

  @Override
  public Boolean update(Db2testIdentityAlwaysRow row, Connection c) {
    var shouldUpdate =
        Optional.ofNullable(map.get(row.id())).filter(oldRow -> !oldRow.equals(row)).isPresent();
    if (shouldUpdate) {
      map.put(row.id(), row);
      ;
    }
    ;
    return shouldUpdate;
  }
  ;

  @Override
  public void upsert(Db2testIdentityAlwaysRow unsaved, Connection c) {
    map.put(unsaved.id(), unsaved);
  }
  ;

  @Override
  public void upsertBatch(Iterator<Db2testIdentityAlwaysRow> unsaved, Connection c) {
    while (unsaved.hasNext()) {
      var row = unsaved.next();
      map.put(row.id(), row);
    }
    ;
  }
  ;
}
