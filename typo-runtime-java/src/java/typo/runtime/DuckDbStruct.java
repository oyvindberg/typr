 ...
 * <p>
 * A STRUCT is an ordered sequence of named fields with typed values.
 * Example: STRUCT(name VARCHAR, age INTEGER)
 * <p>
 * In Java, we represent a STRUCT as a generated record class with typed fields.
 * This class provides the machinery to read/write STRUCTs via JDBC.
 *
...
public record DuckDbStruct<A>(
    ...
    public static class Builder<A> {
        ...
        /**
         * Add a field with a getter function.
         * DuckDbStringifier is automatically derived from the DuckDbType.
         *
         ...
         * - Writer: uses getters to extract field values
         * - JSON: standard format {"field1": value1, "field2": value2}
         *
         * @param reader function to construct struct from field values array
         */
        public DuckDbStruct<A> build(StructReader<A> reader) {
            List<DuckDbTypename.StructOf.StructField> typenameFields = fields.stream()
            ...
            StructWriter<A> writer = structValue -> {
         ...
         */
        public DuckDbStruct<A> build(StructReader<A> reader, StructWriter<A> writer, DuckDbJson<A> json) {
            List<DuckDbTypename.StructOf.StructField> typenameFields = fields.stream()
                    .map(f -> new DuckDbTypename.StructOf.StructField(f.name(), f.type().typename()))
                    .toList();
