...
/**
 * Describes how to read a column from a {@link ResultSet} for DuckDB.
 * DuckDB's JDBC driver provides good type support for most types.
 */
public sealed interface DuckDbRead<A> extends DbRead<A> permits DuckDbRead.NonNullable, DuckDbRead.Nullable {
    ...
    DuckDbRead<LocalDateTime> readLocalDateTime = of((rs, idx) -> {
    ...
    DuckDbRead<OffsetDateTime> readOffsetDateTime = of((rs, idx) -> {
        Object obj = rs.getObject(idx);
        if (obj == null) return null;
    ...
    DuckDbRead<UUID> readUuid = of((rs, idx) -> {
        Object obj = rs.getObject(idx);
        if (obj == null) return null;
    ...
    DuckDbRead<Duration> readDuration = of((rs, idx) -> {
    ...
    DuckDbRead<byte[]> readBlob = of((rs, idx) -> {
    ...
    /**
     * Read a LIST/Array column. DuckDB returns org.duckdb.DuckDBArray which implements java.sql.Array.
     * The elements are extracted as a Java array.
     *
    ...
    /**
     * Read a LIST/Array column with element conversion.
     * Use this when DuckDB returns elements in a different type than expected
     * (e.g., java.sql.Timestamp instead of LocalDateTime).
     *
     ...
     * @return reader for List of elements
     */
    static <E, W> DuckDbRead<java.util.List<E>> readListConverted(java.util.function.Function<W, E> converter) {
        return of((rs, idx) -> {
            java.sql.Array arr = rs.getArray(idx);
            if (arr == null) return null;
            Object[] elements = (Object[]) arr.getArray();
     ...
     */
    DuckDbRead<java.util.Map<String, Object>> readStruct = of((rs, idx) -> {
        Object obj = rs.getObject(idx);
        if (obj == null) return null;
    ...
    /**
     * Read a MAP column with typed keys and values.
     * DuckDB returns java.util.HashMap directly with proper types.
     *
    ...
    static <K, V> DuckDbRead<java.util.Map<K, V>> readMap(
            DuckDbRead<K> keyReader,
            Class<K> keyClass,
            DuckDbRead<V> valueReader,
            Class<V> valueClass
    ) {
        return of((rs, idx) -> {
            Object obj = rs.getObject(idx);
    ...
    /**
     * Parse field names from a STRUCT type definition.
