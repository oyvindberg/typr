 ...
 * <p>
 * A UNION is a tagged union (sum type) that can hold one of several alternative values.
 * Each alternative has a tag name and a type. The current tag is tracked internally.
 * Example: UNION(num INTEGER, str VARCHAR)
 * <p>
 * In Java, we represent a UNION as a sealed interface with record implementations
 * for each alternative. This class provides the machinery to read/write UNIONs via JDBC.
 * <p>
 * Key insight from DuckDB JDBC: UNION values are returned as the actual value (unwrapped),
 * not as a Struct. The tag can be queried via union_tag(value) function.
 *
...
public record DuckDbUnion<A>(
     ...
     * <p>
     * The wrapper converts from the raw value to the union variant.
     * The unwrapper extracts the raw value from a union variant (returns null if wrong variant).
     *
    ...
    public record Member<A, M>(
        ...
        /**
         * Try to unwrap this member from a union value.
         * Returns null if the union value is not of this member's variant.
         */
    ...
    /**
     * Functional interface for reading a UNION from tag + value.
     * The reader receives the tag name and the raw value (already converted by JDBC).
     */
    @FunctionalInterface
    ...
    public DuckDbType<A> asType() {
        DuckDbRead<A> duckDbRead = DuckDbRead.of((rs, idx) -> {
            ...
            throw new SQLException("Cannot determine UNION tag for value: " + obj + " (" + obj.getClass() + ")");
        });

        DuckDbWrite<A> duckDbWrite = new DuckDbWrite.Instance<>(
                DuckDbUnion::setTaggedValue,
                writer::write
        );

        DuckDbStringifier<A> stringifier = DuckDbStringifier.instance((value, sb, quoted) -> {
    ...
    private static <V> void setTaggedValue(java.sql.PreparedStatement ps, int idx, TaggedValue<V> tv) throws java.sql.SQLException {
    ...
    public static class Builder<A> {
         ...
         * <p>
         * The wrapper creates the union variant from a raw value.
         * The unwrapper extracts the raw value (or null if wrong variant).
         *
         ...
         * - Reader: infers tag from Java type, applies appropriate wrapper
         * - Writer: tries each unwrapper until one succeeds
         * - JSON: standard format {"tag": "name", "value": ...}
         */
        public DuckDbUnion<A> build() {
            List<DuckDbTypename.UnionOf.UnionMember> typenameMembers = members.stream()
            ...
            UnionReader<A> reader = (tag, value) -> {
                for (Member<A, ?> member : members) {
            ...
            UnionWriter<A> writer = unionValue -> {
                for (Member<A, ?> member : members) {
            ...
            DuckDbJson<A> json = new DuckDbJson<>() {
                ...
                public A fromJson(JsonValue json) {
                    ...
                    throw new IllegalArgumentException("Expected JSON object with 'tag' and 'value' fields");
                }
            };
        ...
        private <M> JsonValue memberToJson(Member<A, M> member, Object value) {
            ...
            return new JsonValue.JObject(java.util.Map.of(
                    "tag", new JsonValue.JString(member.tag()),
                    "value", innerJson
            ));
        }
        ...
        /**
         * Build with custom reader, writer, and JSON codec.
         * Use this when auto-derivation doesn't work for your use case.
         */
