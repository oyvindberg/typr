 ...
 * <p>
 * This is used for writing complex nested structures (STRUCT, MAP, LIST) via PreparedStatement
 * by converting them to SQL literal strings that DuckDB can parse.
 * <p>
 * Examples:
 * - VARCHAR: 'hello' (quoted, escaped) or hello (unquoted for arrays)
 * - INTEGER: 42 (always unquoted)
 * - STRUCT: {'name': 'Alice', 'age': 30}
 * - LIST: [1, 2, 3]
 * - MAP: {'key1': 'value1', 'key2': 'value2'}
 * <p>
 * The quoted parameter controls whether string-like types include quotes.
 * Use quoted=true for STRUCT field values, quoted=false for array/map elements.
 */
public abstract class DuckDbStringifier<A> {
     ...
     * Convenience method that creates a StringBuilder internally.
     */
    ...
    public static final DuckDbStringifier<String> string = instance((s, sb, quoted) -> {
        if (quoted) {
            sb.append("'");
            sb.append(s.replace("'", "''"));
            sb.append("'");
    ...
    public static final DuckDbStringifier<Boolean> bool = instance((b, sb, quoted) -> sb.append(b ? "true" : "false"));
    ...
    public static final DuckDbStringifier<Integer> integer = instance((n, sb, quoted) -> sb.append(n));
    public static final DuckDbStringifier<Long> bigint = instance((n, sb, quoted) -> sb.append(n));
    public static final DuckDbStringifier<BigInteger> hugeint = instance((n, sb, quoted) -> sb.append(n.toString()));
    ...
    public static final DuckDbStringifier<BigDecimal> numeric = instance((n, sb, quoted) -> sb.append(n.toPlainString()));

    // ==================== Date/Time types (quoted or unquoted) ====================

    public static final DuckDbStringifier<LocalDate> date = instance((d, sb, quoted) -> {
        if (quoted) sb.append("'");
        sb.append(d);
        if (quoted) sb.append("'");
    });

    public static final DuckDbStringifier<LocalTime> time = instance((t, sb, quoted) -> {
        if (quoted) sb.append("'");
        sb.append(t);
        if (quoted) sb.append("'");
    });

    public static final DuckDbStringifier<LocalDateTime> timestamp = instance((ts, sb, quoted) -> {
        if (quoted) sb.append("'");
        sb.append(ts);
        if (quoted) sb.append("'");
    });

    public static final DuckDbStringifier<OffsetDateTime> timestamptz = instance((ts, sb, quoted) -> {
        if (quoted) sb.append("'");
        sb.append(ts);
        if (quoted) sb.append("'");
    });

    public static final DuckDbStringifier<Duration> interval = instance((d, sb, quoted) -> {
        if (quoted) sb.append("'");
    ...
    public static final DuckDbStringifier<UUID> uuid = instance((u, sb, quoted) -> {
        if (quoted) sb.append("'");
        sb.append(u);
        if (quoted) sb.append("'");
    });
    ...
    public static final DuckDbStringifier<byte[]> blob = instance((bytes, sb, quoted) -> {
        sb.append("'\\x");
        for (byte b : bytes) {
