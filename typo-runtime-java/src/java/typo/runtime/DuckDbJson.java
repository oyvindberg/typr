...
/**
 * DuckDB-specific JSON codec implementations.
 * DuckDB has native JSON support and can output results as JSON.
 */
public interface DuckDbJson<A> extends DbJson<A> {
    ...
    DuckDbJson<Boolean> bool = new DuckDbJson<>() {
        @Override
        ...
        public Boolean fromJson(JsonValue json) {
            ...
            if (json instanceof JsonValue.JNumber(BigDecimal value)) return value.intValue() != 0;
            throw new IllegalArgumentException("Expected boolean or number, got: " + json.getClass().getSimpleName());
        }
    };

    DuckDbJson<Byte> int1 = new DuckDbJson<>() {
        @Override
        ...
        public Byte fromJson(JsonValue json) {
            if (json instanceof JsonValue.JNumber(BigDecimal value)) return value.byteValueExact();
            throw new IllegalArgumentException("Expected number, got: " + json.getClass().getSimpleName());
        }
    };

    DuckDbJson<Short> int2 = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<Integer> int4 = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<Long> int8 = new DuckDbJson<>() {
        @Override
    ...
    // HUGEINT (128-bit) - represented as BigInteger, serialized as string in JSON to avoid precision loss
    DuckDbJson<BigInteger> hugeint = new DuckDbJson<>() {
        @Override
        ...
        public BigInteger fromJson(JsonValue json) {
            ...
            if (json instanceof JsonValue.JNumber(BigDecimal value)) return value.toBigInteger();
            throw new IllegalArgumentException("Expected string or number for hugeint, got: " + json.getClass().getSimpleName());
        }
    };

    DuckDbJson<Float> float4 = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<Double> float8 = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<BigDecimal> numeric = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<String> text = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<byte[]> blob = new DuckDbJson<>() {
        @Override
        public JsonValue toJson(byte[] value) {
        ...
        public byte[] fromJson(JsonValue json) {
            if (!(json instanceof JsonValue.JString(String value))) {
                throw new IllegalArgumentException("Expected string for blob, got: " + json.getClass().getSimpleName());
            }
            // Try base64 first
            try {
    ...
    // Date/Time types
    DuckDbJson<LocalDate> date = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<LocalTime> time = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<LocalDateTime> timestamp = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<OffsetDateTime> timestamptz = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<Duration> interval = new DuckDbJson<>() {
        @Override
        ...
        public Duration fromJson(JsonValue json) {
            ...
            throw new IllegalArgumentException("Expected string for interval, got: " + json.getClass().getSimpleName());
        }
    };

    DuckDbJson<UUID> uuid = new DuckDbJson<>() {
        @Override
    ...
    DuckDbJson<Json> json = new DuckDbJson<>() {
        @Override
