...
/**
 * Describes how to write a value to a {@link PreparedStatement} for DuckDB.
 * DuckDB's JDBC driver handles most types through setObject, but some types
 * need special handling:
 * - UUID: use setString to avoid byte ordering bug in setObject
 * - TIME: use setString to avoid timezone issues with java.sql.Time
 * - INTERVAL: use setString with duration format
 */
public sealed interface DuckDbWrite<A> extends DbWrite<A> permits DuckDbWrite.Instance {
    ...
    DuckDbWrite<Duration> writeDuration = writeString.contramap(d -> {
    ...
    /**
     * Write a LIST/Array by converting to DuckDBUserArray.
     * DuckDB JDBC natively supports DuckDBUserArray via setObject().
     *
     ...
     */
    static <E> DuckDbWrite<java.util.List<E>> writeList(String typeName, java.util.function.IntFunction<E[]> toArray) {
        return primitive((ps, idx, list) -> {
            if (list == null) {
                ps.setNull(idx, java.sql.Types.ARRAY);
            } else {
                E[] array = list.toArray(toArray);
                org.duckdb.user.DuckDBUserArray userArray = new org.duckdb.user.DuckDBUserArray(typeName, array);
    ...
    /**
     * Write a LIST/Array by formatting elements using DuckDbStringifier.
     * Use this for types that DuckDB JNI doesn't handle natively.
     * Uses unquoted format (quoted=false) suitable for DuckDBUserArray.
     *
     ...
     * @return writer for List of elements
     */
    static <E> DuckDbWrite<java.util.List<E>> writeListViaSqlLiteral(String typeName, DuckDbStringifier<E> stringifier) {
        return primitive((ps, idx, list) -> {
            if (list == null) {
                ps.setNull(idx, java.sql.Types.ARRAY);
            } else {
                String[] array = list.stream()
                        .map(e -> stringifier.encode(e, false))
                        .toArray(String[]::new);
                org.duckdb.user.DuckDBUserArray userArray = new org.duckdb.user.DuckDBUserArray(typeName, array);
                ps.setObject(idx, userArray);
            }
    ...
    /**
     * Write a MAP with typed keys and values using DuckDBMap.
     * DuckDB JDBC natively supports DuckDBMap via setObject().
     *
    ...
    static <K, V> DuckDbWrite<java.util.Map<K, V>> writeMap(String sqlTypeName) {
        return primitive((ps, idx, map) -> {
            if (map == null) {
                ps.setNull(idx, java.sql.Types.OTHER);
            } else {
                org.duckdb.user.DuckDBMap<K, V> duckDbMap = new org.duckdb.user.DuckDBMap<>(sqlTypeName, map);
    ...
    /**
     * Write a MAP with typed keys and values using DuckDBMap, converting via DuckDbStringifier.
     * All keys and values are converted to String. DuckDB parses them based on the type name.
     * Uses unquoted format (quoted=false) suitable for DuckDBMap.
     *
    ...
    static <K, V> DuckDbWrite<java.util.Map<K, V>> writeMapViaSqlLiteral(
        ...
        return primitive((ps, idx, map) -> {
            if (map == null) {
                ...
                org.duckdb.user.DuckDBMap<String, String> duckDbMap = new org.duckdb.user.DuckDBMap<>(sqlTypeName, wireMap);
                ps.setObject(idx, duckDbMap);
            }
        });
    }
