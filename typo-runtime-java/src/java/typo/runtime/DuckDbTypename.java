...
/**
 * Describes the SQL type name for DuckDB types.
 * DuckDB has a rich type system including composite types like LIST, MAP, etc.
 * This is a sealed interface with specific implementations for each type structure.
 */
public sealed interface DuckDbTypename<A> extends DbTypename<A> {
    ...
    /**
     * Create a LIST type from this element type.
     * For example, VARCHAR becomes VARCHAR[] (or LIST(VARCHAR)).
     */
    DuckDbTypename<java.util.List<A>> list();

    /**
     * Create a MAP type from this key type and another value type.
     * For example, VARCHAR.mapTo(INTEGER) becomes MAP(VARCHAR, INTEGER).
     */
    ...
    /**
     * Base type with optional precision and scale.
     * Examples: VARCHAR, DECIMAL(10,2), INTEGER
     */
    record Base<A>(String baseType, Optional<Integer> precision, Optional<Integer> scale) implements DuckDbTypename<A> {
    ...
    /**
     * LIST type wrapping an element type. LIST is ALWAYS variable-length.
     * Rendered as "element_type[]" (e.g., VARCHAR[] or INTEGER[])
     * <p>
     * Use ARRAY for fixed-length arrays (e.g., embedding vectors).
     */
    record ListOf<A>(DuckDbTypename<A> elementType) implements DuckDbTypename<java.util.List<A>> {
        ...
        @Override
        public <V> DuckDbTypename<java.util.Map<java.util.List<A>, V>> mapTo(DuckDbTypename<V> valueType) {
            return new MapOf<>(this, valueType);
        }
     ...
     * Rendered as "MAP(key_type, value_type)" (e.g., MAP(VARCHAR, INTEGER))
     */
    record MapOf<K, V>(DuckDbTypename<K> keyType, DuckDbTypename<V> valueType) implements DuckDbTypename<java.util.Map<K, V>> {
        @Override
        public String sqlType() {
        ...
        @Override
        public <V2> DuckDbTypename<java.util.Map<java.util.Map<K, V>, V2>> mapTo(DuckDbTypename<V2> valueType2) {
    ...
    /**
     * Fixed-size ARRAY type wrapping an element type. ARRAY is ALWAYS fixed-length.
     * Rendered as "element_type[size]" (e.g., FLOAT[3] for 3D vectors)
     * <p>
     * Use LIST for variable-length arrays.
     * <p>
     * From DuckDB docs: "All fields in the column must have the same length."
     * Typically used for embedding vectors (word embeddings, image embeddings).
     */
    record ArrayOf<A>(DuckDbTypename<A> elementType, int size) implements DuckDbTypename<java.util.List<A>> {
        @Override
        public String sqlType() {
     ...
     * Rendered as "STRUCT(field1 type1, field2 type2, ...)"
     */
    ...
    /**
     * UNION type with tagged alternatives.
     * Rendered as "UNION(tag1 type1, tag2 type2, ...)"
     */
     ...
     * The SQL type is the same as the underlying type.
     */
