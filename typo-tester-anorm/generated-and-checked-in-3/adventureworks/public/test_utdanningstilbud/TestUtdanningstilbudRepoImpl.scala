...
class TestUtdanningstilbudRepoImpl extends TestUtdanningstilbudRepo {
  ...
  def deleteById(compositeId: TestUtdanningstilbudId)(using c: Connection): Boolean = SQL"""delete from "public"."test_utdanningstilbud" where "organisasjonskode" = ${ParameterValue(compositeId.organisasjonskode, null, TestOrganisasjonId.toStatement)} AND "utdanningsmulighet_kode" = ${ParameterValue(compositeId.utdanningsmulighetKode, null, ToStatement.stringToStatement)}""".executeUpdate() > 0
