...
class SalesterritoryRepoImpl extends SalesterritoryRepo {
  ...
  def deleteById(territoryid: SalesterritoryId)(using c: Connection): Boolean = SQL"""delete from "sales"."salesterritory" where "territoryid" = ${ParameterValue(territoryid, null, SalesterritoryId.toStatement)}""".executeUpdate() > 0
