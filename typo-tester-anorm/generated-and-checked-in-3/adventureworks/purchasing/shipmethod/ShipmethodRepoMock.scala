/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package adventureworks.purchasing.shipmethod

import java.sql.Connection
import scala.annotation.nowarn
import typo.dsl.DeleteBuilder
import typo.dsl.DeleteBuilder.DeleteBuilderMock
import typo.dsl.DeleteParams
import typo.dsl.SelectBuilder
import typo.dsl.SelectBuilderMock
import typo.dsl.SelectParams
import typo.dsl.UpdateBuilder
import typo.dsl.UpdateBuilder.UpdateBuilderMock
import typo.dsl.UpdateParams

case class ShipmethodRepoMock(
  toRow: ShipmethodRowUnsaved => ShipmethodRow,
  map: scala.collection.mutable.Map[ShipmethodId, ShipmethodRow] = scala.collection.mutable.Map.empty[ShipmethodId, ShipmethodRow]
) extends ShipmethodRepo {
  def delete: DeleteBuilder[ShipmethodFields, ShipmethodRow] = DeleteBuilderMock(DeleteParams.empty, ShipmethodFields.structure, map)

  def deleteById(shipmethodid: ShipmethodId)(using c: Connection): Boolean = map.remove(shipmethodid).isDefined

  def deleteByIds(shipmethodids: Array[ShipmethodId])(using c: Connection): Int = shipmethodids.map(id => map.remove(id)).count(_.isDefined)

  def insert(unsaved: ShipmethodRow)(using c: Connection): ShipmethodRow = {
    val _ = if (map.contains(unsaved.shipmethodid))
      sys.error(s"id ${unsaved.shipmethodid} already exists")
    else
      map.put(unsaved.shipmethodid, unsaved)
  
    unsaved
  }

  def insert(unsaved: ShipmethodRowUnsaved)(using c: Connection): ShipmethodRow = insert(toRow(unsaved))

  def insertStreaming(
    unsaved: Iterator[ShipmethodRow],
    batchSize: Int = 10000
  )(using c: Connection): Long = {
    unsaved.foreach { row =>
      map += (row.shipmethodid -> row)
    }
    unsaved.size.toLong
  }

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(
    unsaved: Iterator[ShipmethodRowUnsaved],
    batchSize: Int = 10000
  )(using c: Connection): Long = {
    unsaved.foreach { unsavedRow =>
      val row = toRow(unsavedRow)
      map += (row.shipmethodid -> row)
    }
    unsaved.size.toLong
  }

  def select: SelectBuilder[ShipmethodFields, ShipmethodRow] = SelectBuilderMock(ShipmethodFields.structure, () => map.values.toList, SelectParams.empty)

  def selectAll(using c: Connection): List[ShipmethodRow] = map.values.toList

  def selectById(shipmethodid: ShipmethodId)(using c: Connection): Option[ShipmethodRow] = map.get(shipmethodid)

  def selectByIds(shipmethodids: Array[ShipmethodId])(using c: Connection): List[ShipmethodRow] = shipmethodids.flatMap(map.get).toList

  def selectByIdsTracked(shipmethodids: Array[ShipmethodId])(using c: Connection): Map[ShipmethodId, ShipmethodRow] = {
    val byId = selectByIds(shipmethodids).view.map(x => (x.shipmethodid, x)).toMap
    shipmethodids.view.flatMap(id => byId.get(id).map(x => (id, x))).toMap
  }

  def update: UpdateBuilder[ShipmethodFields, ShipmethodRow] = UpdateBuilderMock(UpdateParams.empty, ShipmethodFields.structure, map)

  def update(row: ShipmethodRow)(using c: Connection): Option[ShipmethodRow] = {
    map.get(row.shipmethodid).map { _ =>
      map.put(row.shipmethodid, row): @nowarn
      row
    }
  }

  def upsert(unsaved: ShipmethodRow)(using c: Connection): ShipmethodRow = {
    map.put(unsaved.shipmethodid, unsaved): @nowarn
    unsaved
  }

  def upsertBatch(unsaved: Iterable[ShipmethodRow])(using c: Connection): List[ShipmethodRow] = {
    unsaved.map { row =>
      map += (row.shipmethodid -> row)
      row
    }.toList
  }

  /** NOTE: this functionality is not safe if you use auto-commit mode! it runs 3 SQL statements */
  def upsertStreaming(
    unsaved: Iterator[ShipmethodRow],
    batchSize: Int = 10000
  )(using c: Connection): Int = {
    unsaved.foreach { row =>
      map += (row.shipmethodid -> row)
    }
    unsaved.size
  }
}