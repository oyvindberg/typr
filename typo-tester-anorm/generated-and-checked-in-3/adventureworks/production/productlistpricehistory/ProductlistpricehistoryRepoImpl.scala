...
class ProductlistpricehistoryRepoImpl extends ProductlistpricehistoryRepo {
  ...
  def deleteById(compositeId: ProductlistpricehistoryId)(using c: Connection): Boolean = SQL"""delete from "production"."productlistpricehistory" where "productid" = ${ParameterValue(compositeId.productid, null, ProductId.toStatement)} AND "startdate" = ${ParameterValue(compositeId.startdate, null, TypoLocalDateTime.toStatement)}""".executeUpdate() > 0
