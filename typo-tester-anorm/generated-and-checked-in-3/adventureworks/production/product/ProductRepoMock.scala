...
case class ProductRepoMock(
  ...
  def deleteById(productid: ProductId)(using c: Connection): Boolean = map.remove(productid).isDefined

  def deleteByIds(productids: Array[ProductId])(using c: Connection): Int = productids.map(id => map.remove(id)).count(_.isDefined)
  ...
  def selectById(productid: ProductId)(using c: Connection): Option[ProductRow] = map.get(productid)

  def selectByIds(productids: Array[ProductId])(using c: Connection): List[ProductRow] = productids.flatMap(map.get).toList
