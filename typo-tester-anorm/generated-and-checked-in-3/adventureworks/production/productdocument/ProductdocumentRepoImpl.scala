...
class ProductdocumentRepoImpl extends ProductdocumentRepo {
  ...
  def deleteById(compositeId: ProductdocumentId)(using c: Connection): Boolean = SQL"""delete from "production"."productdocument" where "productid" = ${ParameterValue(compositeId.productid, null, ProductId.toStatement)} AND "documentnode" = ${ParameterValue(compositeId.documentnode, null, DocumentId.toStatement)}""".executeUpdate() > 0
