...
class WorkorderRepoImpl extends WorkorderRepo {
  ...
  def deleteById(workorderid: WorkorderId)(using c: Connection): Boolean = SQL"""delete from "production"."workorder" where "workorderid" = ${ParameterValue(workorderid, null, WorkorderId.toStatement)}""".executeUpdate() > 0
