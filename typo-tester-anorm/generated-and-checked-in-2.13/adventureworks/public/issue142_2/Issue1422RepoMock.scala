...
case class Issue1422RepoMock(map: scala.collection.mutable.Map[Issue142Id, Issue1422Row] = scala.collection.mutable.Map.empty[Issue142Id, Issue1422Row]) extends Issue1422Repo {
  ...
  def selectAll(implicit c: Connection): List[Issue1422Row] = map.values.toList

  def selectById(tabellkode: Issue142Id)(implicit c: Connection): Option[Issue1422Row] = map.get(tabellkode)

  def selectByIds(tabellkodes: Array[Issue142Id])(implicit c: Connection): List[Issue1422Row] = tabellkodes.flatMap(map.get).toList
