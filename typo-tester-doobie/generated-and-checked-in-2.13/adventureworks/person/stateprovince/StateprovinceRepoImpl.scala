...
class StateprovinceRepoImpl extends StateprovinceRepo {
  ...
  def deleteById(stateprovinceid: StateprovinceId): ConnectionIO[Boolean] = sql"""delete from "person"."stateprovince" where "stateprovinceid" = ${fromWrite(stateprovinceid)(new Write.Single(StateprovinceId.put))}""".update.run.map(_ > 0)

  def deleteByIds(stateprovinceids: Array[StateprovinceId]): ConnectionIO[Int] = sql"""delete from "person"."stateprovince" where "stateprovinceid" = ANY(${fromWrite(stateprovinceids)(new Write.Single(StateprovinceId.arrayPut))})""".update.run
  ...
  def selectAll: Stream[ConnectionIO, StateprovinceRow] = sql"""select "stateprovinceid", "stateprovincecode", "countryregioncode", "isonlystateprovinceflag", "name", "territoryid", "rowguid", "modifieddate"::text from "person"."stateprovince"""".query(StateprovinceRow.read).stream
