...
class AddressRepoImpl extends AddressRepo {
  ...
  override def deleteByIds(addressids: Array[AddressId]): ConnectionIO[Int] = {
    sql"""delete from "person"."address" where "addressid" = ANY(${fromWrite(addressids)(new Write.Single(AddressId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(addressids: Array[AddressId]): Stream[ConnectionIO, AddressRow] = {
    sql"""select "addressid", "addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "rowguid", "modifieddate"::text from "person"."address" where "addressid" = ANY(${fromWrite(addressids)(new Write.Single(AddressId.arrayPut))})""".query(AddressRow.read).stream
  }
