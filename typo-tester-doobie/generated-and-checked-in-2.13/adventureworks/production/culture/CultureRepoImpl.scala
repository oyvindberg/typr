...
class CultureRepoImpl extends CultureRepo {
  ...
  override def deleteByIds(cultureids: Array[CultureId]): ConnectionIO[Int] = {
    sql"""delete from "production"."culture" where "cultureid" = ANY(${fromWrite(cultureids)(new Write.Single(CultureId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(cultureids: Array[CultureId]): Stream[ConnectionIO, CultureRow] = {
    sql"""select "cultureid", "name", "modifieddate"::text from "production"."culture" where "cultureid" = ANY(${fromWrite(cultureids)(new Write.Single(CultureId.arrayPut))})""".query(CultureRow.read).stream
  }
