...
class BillofmaterialsRepoImpl extends BillofmaterialsRepo {
  ...
  def deleteById(billofmaterialsid: Int): ConnectionIO[Boolean] = sql"""delete from "production"."billofmaterials" where "billofmaterialsid" = ${fromWrite(billofmaterialsid)(new Write.Single(Meta.IntMeta.put))}""".update.run.map(_ > 0)

  def deleteByIds(billofmaterialsids: Array[Int]): ConnectionIO[Int] = sql"""delete from "production"."billofmaterials" where "billofmaterialsid" = ANY(${fromWrite(billofmaterialsids)(new Write.Single(adventureworks.IntegerArrayMeta.put))})""".update.run
  ...
  def selectAll: Stream[ConnectionIO, BillofmaterialsRow] = sql"""select "billofmaterialsid", "productassemblyid", "componentid", "startdate"::text, "enddate"::text, "unitmeasurecode", "bomlevel", "perassemblyqty", "modifieddate"::text from "production"."billofmaterials"""".query(BillofmaterialsRow.read).stream
