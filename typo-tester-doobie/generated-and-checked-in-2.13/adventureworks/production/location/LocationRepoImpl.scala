...
class LocationRepoImpl extends LocationRepo {
  ...
  override def deleteByIds(locationids: Array[LocationId]): ConnectionIO[Int] = {
    sql"""delete from "production"."location" where "locationid" = ANY(${fromWrite(locationids)(new Write.Single(LocationId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(locationids: Array[LocationId]): Stream[ConnectionIO, LocationRow] = {
    sql"""select "locationid", "name", "costrate", "availability", "modifieddate"::text from "production"."location" where "locationid" = ANY(${fromWrite(locationids)(new Write.Single(LocationId.arrayPut))})""".query(LocationRow.read).stream
  }
