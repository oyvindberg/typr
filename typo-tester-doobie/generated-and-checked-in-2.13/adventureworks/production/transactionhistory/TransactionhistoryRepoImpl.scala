...
class TransactionhistoryRepoImpl extends TransactionhistoryRepo {
  ...
  override def deleteByIds(transactionids: Array[TransactionhistoryId]): ConnectionIO[Int] = {
    sql"""delete from "production"."transactionhistory" where "transactionid" = ANY(${fromWrite(transactionids)(new Write.Single(TransactionhistoryId.arrayPut))})""".update.run
  }
