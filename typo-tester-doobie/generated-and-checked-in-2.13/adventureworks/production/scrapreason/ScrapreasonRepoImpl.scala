...
class ScrapreasonRepoImpl extends ScrapreasonRepo {
  ...
  def deleteById(scrapreasonid: ScrapreasonId): ConnectionIO[Boolean] = sql"""delete from "production"."scrapreason" where "scrapreasonid" = ${fromWrite(scrapreasonid)(new Write.Single(ScrapreasonId.put))}""".update.run.map(_ > 0)

  def deleteByIds(scrapreasonids: Array[ScrapreasonId]): ConnectionIO[Int] = sql"""delete from "production"."scrapreason" where "scrapreasonid" = ANY(${fromWrite(scrapreasonids)(new Write.Single(ScrapreasonId.arrayPut))})""".update.run
  ...
  def selectAll: Stream[ConnectionIO, ScrapreasonRow] = sql"""select "scrapreasonid", "name", "modifieddate"::text from "production"."scrapreason"""".query(ScrapreasonRow.read).stream

  def selectById(scrapreasonid: ScrapreasonId): ConnectionIO[Option[ScrapreasonRow]] = sql"""select "scrapreasonid", "name", "modifieddate"::text from "production"."scrapreason" where "scrapreasonid" = ${fromWrite(scrapreasonid)(new Write.Single(ScrapreasonId.put))}""".query(ScrapreasonRow.read).option

  def selectByIds(scrapreasonids: Array[ScrapreasonId]): Stream[ConnectionIO, ScrapreasonRow] = sql"""select "scrapreasonid", "name", "modifieddate"::text from "production"."scrapreason" where "scrapreasonid" = ANY(${fromWrite(scrapreasonids)(new Write.Single(ScrapreasonId.arrayPut))})""".query(ScrapreasonRow.read).stream
