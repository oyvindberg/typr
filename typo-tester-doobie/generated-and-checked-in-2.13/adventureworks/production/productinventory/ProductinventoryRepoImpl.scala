...
class ProductinventoryRepoImpl extends ProductinventoryRepo {
  ...
  def deleteById(compositeId: ProductinventoryId): ConnectionIO[Boolean] = sql"""delete from "production"."productinventory" where "productid" = ${fromWrite(compositeId.productid)(new Write.Single(ProductId.put))} AND "locationid" = ${fromWrite(compositeId.locationid)(new Write.Single(LocationId.put))}""".update.run.map(_ > 0)
  ...
  def selectAll: Stream[ConnectionIO, ProductinventoryRow] = sql"""select "productid", "locationid", "shelf", "bin", "quantity", "rowguid", "modifieddate"::text from "production"."productinventory"""".query(ProductinventoryRow.read).stream
