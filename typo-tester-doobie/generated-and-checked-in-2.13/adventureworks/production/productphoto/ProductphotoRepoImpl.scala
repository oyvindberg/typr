...
class ProductphotoRepoImpl extends ProductphotoRepo {
  ...
  def deleteById(productphotoid: ProductphotoId): ConnectionIO[Boolean] = sql"""delete from "production"."productphoto" where "productphotoid" = ${fromWrite(productphotoid)(new Write.Single(ProductphotoId.put))}""".update.run.map(_ > 0)

  def deleteByIds(productphotoids: Array[ProductphotoId]): ConnectionIO[Int] = sql"""delete from "production"."productphoto" where "productphotoid" = ANY(${fromWrite(productphotoids)(new Write.Single(ProductphotoId.arrayPut))})""".update.run
  ...
  def selectAll: Stream[ConnectionIO, ProductphotoRow] = sql"""select "productphotoid", "thumbnailphoto", "thumbnailphotofilename", "largephoto", "largephotofilename", "modifieddate"::text from "production"."productphoto"""".query(ProductphotoRow.read).stream
