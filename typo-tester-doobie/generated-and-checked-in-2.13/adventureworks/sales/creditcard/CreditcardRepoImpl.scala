...
class CreditcardRepoImpl extends CreditcardRepo {
  def delete: DeleteBuilder[CreditcardFields, CreditcardRow] = DeleteBuilder.of(""""sales"."creditcard"""", CreditcardFields.structure, CreditcardRow.read)

  def deleteById(creditcardid: /* user-picked */ CustomCreditcardId): ConnectionIO[Boolean] = sql"""delete from "sales"."creditcard" where "creditcardid" = ${fromWrite(creditcardid)(new Write.Single(/* user-picked */ CustomCreditcardId.put))}""".update.run.map(_ > 0)

  def deleteByIds(creditcardids: Array[/* user-picked */ CustomCreditcardId]): ConnectionIO[Int] = sql"""delete from "sales"."creditcard" where "creditcardid" = ANY(${fromWrite(creditcardids)(new Write.Single(CustomCreditcardId.arrayPut))})""".update.run
  ...
  def select: SelectBuilder[CreditcardFields, CreditcardRow] = SelectBuilder.of(""""sales"."creditcard"""", CreditcardFields.structure, CreditcardRow.read)

  def selectAll: Stream[ConnectionIO, CreditcardRow] = sql"""select "creditcardid", "cardtype", "cardnumber", "expmonth", "expyear", "modifieddate"::text from "sales"."creditcard"""".query(CreditcardRow.read).stream
  ...
  def update: UpdateBuilder[CreditcardFields, CreditcardRow] = UpdateBuilder.of(""""sales"."creditcard"""", CreditcardFields.structure, CreditcardRow.read)
