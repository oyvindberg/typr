...
class ShiftRepoImpl extends ShiftRepo {
  ...
  override def deleteByIds(shiftids: Array[ShiftId]): ConnectionIO[Int] = {
    sql"""delete from "humanresources"."shift" where "shiftid" = ANY(${fromWrite(shiftids)(new Write.Single(ShiftId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(shiftids: Array[ShiftId]): Stream[ConnectionIO, ShiftRow] = {
    sql"""select "shiftid", "name", "starttime"::text, "endtime"::text, "modifieddate"::text from "humanresources"."shift" where "shiftid" = ANY(${fromWrite(shiftids)(new Write.Single(ShiftId.arrayPut))})""".query(ShiftRow.read).stream
  }
