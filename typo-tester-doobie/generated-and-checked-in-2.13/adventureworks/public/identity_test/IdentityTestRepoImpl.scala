...
class IdentityTestRepoImpl extends IdentityTestRepo {
  ...
  def deleteById(name: IdentityTestId): ConnectionIO[Boolean] = sql"""delete from "public"."identity-test" where "name" = ${fromWrite(name)(new Write.Single(IdentityTestId.put))}""".update.run.map(_ > 0)

  def deleteByIds(names: Array[IdentityTestId]): ConnectionIO[Int] = sql"""delete from "public"."identity-test" where "name" = ANY(${fromWrite(names)(new Write.Single(IdentityTestId.arrayPut))})""".update.run
  ...
  def selectAll: Stream[ConnectionIO, IdentityTestRow] = sql"""select "always_generated", "default_generated", "name" from "public"."identity-test"""".query(IdentityTestRow.read).stream

  def selectById(name: IdentityTestId): ConnectionIO[Option[IdentityTestRow]] = sql"""select "always_generated", "default_generated", "name" from "public"."identity-test" where "name" = ${fromWrite(name)(new Write.Single(IdentityTestId.put))}""".query(IdentityTestRow.read).option

  def selectByIds(names: Array[IdentityTestId]): Stream[ConnectionIO, IdentityTestRow] = sql"""select "always_generated", "default_generated", "name" from "public"."identity-test" where "name" = ANY(${fromWrite(names)(new Write.Single(IdentityTestId.arrayPut))})""".query(IdentityTestRow.read).stream
