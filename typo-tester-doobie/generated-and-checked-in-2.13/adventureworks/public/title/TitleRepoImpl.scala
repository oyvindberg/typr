...
class TitleRepoImpl extends TitleRepo {
  ...
  def deleteById(code: TitleId): ConnectionIO[Boolean] = sql"""delete from "public"."title" where "code" = ${fromWrite(code)(new Write.Single(TitleId.put))}""".update.run.map(_ > 0)

  def deleteByIds(codes: Array[TitleId]): ConnectionIO[Int] = sql"""delete from "public"."title" where "code" = ANY(${fromWrite(codes)(new Write.Single(TitleId.arrayPut))})""".update.run
  ...
  def selectAll: Stream[ConnectionIO, TitleRow] = sql"""select "code" from "public"."title"""".query(TitleRow.read).stream

  def selectById(code: TitleId): ConnectionIO[Option[TitleRow]] = sql"""select "code" from "public"."title" where "code" = ${fromWrite(code)(new Write.Single(TitleId.put))}""".query(TitleRow.read).option

  def selectByIds(codes: Array[TitleId]): Stream[ConnectionIO, TitleRow] = sql"""select "code" from "public"."title" where "code" = ANY(${fromWrite(codes)(new Write.Single(TitleId.arrayPut))})""".query(TitleRow.read).stream
