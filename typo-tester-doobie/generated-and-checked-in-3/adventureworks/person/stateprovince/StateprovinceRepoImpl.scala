...
class StateprovinceRepoImpl extends StateprovinceRepo {
  ...
  override def deleteByIds(stateprovinceids: Array[StateprovinceId]): ConnectionIO[Int] = {
    sql"""delete from "person"."stateprovince" where "stateprovinceid" = ANY(${fromWrite(stateprovinceids)(using new Write.Single(StateprovinceId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(stateprovinceids: Array[StateprovinceId]): Stream[ConnectionIO, StateprovinceRow] = {
    sql"""select "stateprovinceid", "stateprovincecode", "countryregioncode", "isonlystateprovinceflag", "name", "territoryid", "rowguid", "modifieddate"::text from "person"."stateprovince" where "stateprovinceid" = ANY(${fromWrite(stateprovinceids)(using new Write.Single(StateprovinceId.arrayPut))})""".query(using StateprovinceRow.read).stream
  }
