...
class ContacttypeRepoImpl extends ContacttypeRepo {
  ...
  override def deleteByIds(contacttypeids: Array[ContacttypeId]): ConnectionIO[Int] = {
    sql"""delete from "person"."contacttype" where "contacttypeid" = ANY(${fromWrite(contacttypeids)(using new Write.Single(ContacttypeId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(contacttypeids: Array[ContacttypeId]): Stream[ConnectionIO, ContacttypeRow] = {
    sql"""select "contacttypeid", "name", "modifieddate"::text from "person"."contacttype" where "contacttypeid" = ANY(${fromWrite(contacttypeids)(using new Write.Single(ContacttypeId.arrayPut))})""".query(using ContacttypeRow.read).stream
  }
