...
class CountryregioncurrencyRepoImpl extends CountryregioncurrencyRepo {
  ...
  def deleteById(compositeId: CountryregioncurrencyId): ConnectionIO[Boolean] = sql"""delete from "sales"."countryregioncurrency" where "countryregioncode" = ${fromWrite(compositeId.countryregioncode)(using new Write.Single(CountryregionId.put))} AND "currencycode" = ${fromWrite(compositeId.currencycode)(using new Write.Single(CurrencyId.put))}""".update.run.map(_ > 0)
