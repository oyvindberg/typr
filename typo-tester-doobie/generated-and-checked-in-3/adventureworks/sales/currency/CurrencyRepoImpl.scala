...
class CurrencyRepoImpl extends CurrencyRepo {
  ...
  override def deleteByIds(currencycodes: Array[CurrencyId]): ConnectionIO[Int] = {
    sql"""delete from "sales"."currency" where "currencycode" = ANY(${fromWrite(currencycodes)(using new Write.Single(CurrencyId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(currencycodes: Array[CurrencyId]): Stream[ConnectionIO, CurrencyRow] = {
    sql"""select "currencycode", "name", "modifieddate"::text from "sales"."currency" where "currencycode" = ANY(${fromWrite(currencycodes)(using new Write.Single(CurrencyId.arrayPut))})""".query(using CurrencyRow.read).stream
  }
