...
class SalesreasonRepoImpl extends SalesreasonRepo {
  ...
  def deleteById(salesreasonid: SalesreasonId): ConnectionIO[Boolean] = sql"""delete from "sales"."salesreason" where "salesreasonid" = ${fromWrite(salesreasonid)(using new Write.Single(SalesreasonId.put))}""".update.run.map(_ > 0)

  def deleteByIds(salesreasonids: Array[SalesreasonId]): ConnectionIO[Int] = sql"""delete from "sales"."salesreason" where "salesreasonid" = ANY(${fromWrite(salesreasonids)(using new Write.Single(SalesreasonId.arrayPut))})""".update.run
  ...
  def selectById(salesreasonid: SalesreasonId): ConnectionIO[Option[SalesreasonRow]] = sql"""select "salesreasonid", "name", "reasontype", "modifieddate"::text from "sales"."salesreason" where "salesreasonid" = ${fromWrite(salesreasonid)(using new Write.Single(SalesreasonId.put))}""".query(using SalesreasonRow.read).option

  def selectByIds(salesreasonids: Array[SalesreasonId]): Stream[ConnectionIO, SalesreasonRow] = sql"""select "salesreasonid", "name", "reasontype", "modifieddate"::text from "sales"."salesreason" where "salesreasonid" = ANY(${fromWrite(salesreasonids)(using new Write.Single(SalesreasonId.arrayPut))})""".query(using SalesreasonRow.read).stream
