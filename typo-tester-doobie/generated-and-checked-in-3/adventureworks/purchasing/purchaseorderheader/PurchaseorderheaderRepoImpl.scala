...
class PurchaseorderheaderRepoImpl extends PurchaseorderheaderRepo {
  ...
  def deleteById(purchaseorderid: PurchaseorderheaderId): ConnectionIO[Boolean] = sql"""delete from "purchasing"."purchaseorderheader" where "purchaseorderid" = ${fromWrite(purchaseorderid)(using new Write.Single(PurchaseorderheaderId.put))}""".update.run.map(_ > 0)

  def deleteByIds(purchaseorderids: Array[PurchaseorderheaderId]): ConnectionIO[Int] = sql"""delete from "purchasing"."purchaseorderheader" where "purchaseorderid" = ANY(${fromWrite(purchaseorderids)(using new Write.Single(PurchaseorderheaderId.arrayPut))})""".update.run
