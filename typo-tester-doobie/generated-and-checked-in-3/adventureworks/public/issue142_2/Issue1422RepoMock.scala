...
case class Issue1422RepoMock(map: scala.collection.mutable.Map[Issue142Id, Issue1422Row] = scala.collection.mutable.Map.empty[Issue142Id, Issue1422Row]) extends Issue1422Repo {
  ...
  def deleteById(tabellkode: Issue142Id): ConnectionIO[Boolean] = delay(map.remove(tabellkode).isDefined)

  def deleteByIds(tabellkodes: Array[Issue142Id]): ConnectionIO[Int] = delay(tabellkodes.map(id => map.remove(id)).count(_.isDefined))
  ...
  def select: SelectBuilder[Issue1422Fields, Issue1422Row] = SelectBuilderMock(Issue1422Fields.structure, delay(map.values.toList), SelectParams.empty)

  def selectAll: Stream[ConnectionIO, Issue1422Row] = Stream.emits(map.values.toList)

  def selectById(tabellkode: Issue142Id): ConnectionIO[Option[Issue1422Row]] = delay(map.get(tabellkode))

  def selectByIds(tabellkodes: Array[Issue142Id]): Stream[ConnectionIO, Issue1422Row] = Stream.emits(tabellkodes.flatMap(map.get).toList)
