...
class Issue1422RepoImpl extends Issue1422Repo {
  ...
  def deleteById(tabellkode: Issue142Id): ConnectionIO[Boolean] = sql"""delete from "public"."issue142_2" where "tabellkode" = ${fromWrite(tabellkode)(using new Write.Single(Issue142Id.put))}""".update.run.map(_ > 0)

  def deleteByIds(tabellkodes: Array[Issue142Id]): ConnectionIO[Int] = sql"""delete from "public"."issue142_2" where "tabellkode" = ANY(${fromWrite(tabellkodes)(using new Write.Single(Issue142Id.arrayPut))})""".update.run
  ...
  def selectAll: Stream[ConnectionIO, Issue1422Row] = sql"""select "tabellkode" from "public"."issue142_2"""".query(using Issue1422Row.read).stream

  def selectById(tabellkode: Issue142Id): ConnectionIO[Option[Issue1422Row]] = sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ${fromWrite(tabellkode)(using new Write.Single(Issue142Id.put))}""".query(using Issue1422Row.read).option

  def selectByIds(tabellkodes: Array[Issue142Id]): Stream[ConnectionIO, Issue1422Row] = sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ANY(${fromWrite(tabellkodes)(using new Write.Single(Issue142Id.arrayPut))})""".query(using Issue1422Row.read).stream
