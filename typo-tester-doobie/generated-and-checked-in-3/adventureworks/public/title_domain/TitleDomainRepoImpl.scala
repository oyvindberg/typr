...
class TitleDomainRepoImpl extends TitleDomainRepo {
  ...
  override def deleteByIds(codes: Array[TitleDomainId]): ConnectionIO[Int] = {
    sql"""delete from "public"."title_domain" where "code" = ANY(${fromWrite(codes)(using new Write.Single(TitleDomainId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(codes: Array[TitleDomainId]): Stream[ConnectionIO, TitleDomainRow] = {
    sql"""select "code" from "public"."title_domain" where "code" = ANY(${fromWrite(codes)(using new Write.Single(TitleDomainId.arrayPut))})""".query(using TitleDomainRow.read).stream
  }
