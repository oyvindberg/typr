...
class TitleRepoImpl extends TitleRepo {
  ...
  override def deleteByIds(codes: Array[TitleId]): ConnectionIO[Int] = {
    sql"""delete from "public"."title" where "code" = ANY(${fromWrite(codes)(using new Write.Single(TitleId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(codes: Array[TitleId]): Stream[ConnectionIO, TitleRow] = {
    sql"""select "code" from "public"."title" where "code" = ANY(${fromWrite(codes)(using new Write.Single(TitleId.arrayPut))})""".query(using TitleRow.read).stream
  }
