...
class UsersRepoImpl extends UsersRepo {
  ...
  override def deleteByIds(userIds: Array[UsersId]): ConnectionIO[Int] = {
    sql"""delete from "public"."users" where "user_id" = ANY(${fromWrite(userIds)(using new Write.Single(UsersId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(userIds: Array[UsersId]): Stream[ConnectionIO, UsersRow] = {
    sql"""select "user_id", "name", "last_name", "email"::text, "password", "created_at"::text, "verified_on"::text from "public"."users" where "user_id" = ANY(${fromWrite(userIds)(using new Write.Single(UsersId.arrayPut))})""".query(using UsersRow.read).stream
  }
