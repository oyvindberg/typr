...
class CultureRepoImpl extends CultureRepo {
  ...
  def deleteById(cultureid: CultureId): ConnectionIO[Boolean] = sql"""delete from "production"."culture" where "cultureid" = ${fromWrite(cultureid)(using new Write.Single(CultureId.put))}""".update.run.map(_ > 0)

  def deleteByIds(cultureids: Array[CultureId]): ConnectionIO[Int] = sql"""delete from "production"."culture" where "cultureid" = ANY(${fromWrite(cultureids)(using new Write.Single(CultureId.arrayPut))})""".update.run
  ...
  def selectById(cultureid: CultureId): ConnectionIO[Option[CultureRow]] = sql"""select "cultureid", "name", "modifieddate"::text from "production"."culture" where "cultureid" = ${fromWrite(cultureid)(using new Write.Single(CultureId.put))}""".query(using CultureRow.read).option

  def selectByIds(cultureids: Array[CultureId]): Stream[ConnectionIO, CultureRow] = sql"""select "cultureid", "name", "modifieddate"::text from "production"."culture" where "cultureid" = ANY(${fromWrite(cultureids)(using new Write.Single(CultureId.arrayPut))})""".query(using CultureRow.read).stream
