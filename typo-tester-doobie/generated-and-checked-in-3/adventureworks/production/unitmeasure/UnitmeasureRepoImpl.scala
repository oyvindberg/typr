...
class UnitmeasureRepoImpl extends UnitmeasureRepo {
  ...
  override def deleteByIds(unitmeasurecodes: Array[UnitmeasureId]): ConnectionIO[Int] = {
    sql"""delete from "production"."unitmeasure" where "unitmeasurecode" = ANY(${fromWrite(unitmeasurecodes)(using new Write.Single(UnitmeasureId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(unitmeasurecodes: Array[UnitmeasureId]): Stream[ConnectionIO, UnitmeasureRow] = {
    sql"""select "unitmeasurecode", "name", "modifieddate"::text from "production"."unitmeasure" where "unitmeasurecode" = ANY(${fromWrite(unitmeasurecodes)(using new Write.Single(UnitmeasureId.arrayPut))})""".query(using UnitmeasureRow.read).stream
  }
