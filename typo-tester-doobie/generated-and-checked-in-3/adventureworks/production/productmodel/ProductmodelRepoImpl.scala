...
class ProductmodelRepoImpl extends ProductmodelRepo {
  ...
  def deleteById(productmodelid: ProductmodelId): ConnectionIO[Boolean] = sql"""delete from "production"."productmodel" where "productmodelid" = ${fromWrite(productmodelid)(using new Write.Single(ProductmodelId.put))}""".update.run.map(_ > 0)

  def deleteByIds(productmodelids: Array[ProductmodelId]): ConnectionIO[Int] = sql"""delete from "production"."productmodel" where "productmodelid" = ANY(${fromWrite(productmodelids)(using new Write.Single(ProductmodelId.arrayPut))})""".update.run
