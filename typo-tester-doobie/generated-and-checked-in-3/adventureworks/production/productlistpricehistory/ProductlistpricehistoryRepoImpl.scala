...
class ProductlistpricehistoryRepoImpl extends ProductlistpricehistoryRepo {
  ...
  def deleteById(compositeId: ProductlistpricehistoryId): ConnectionIO[Boolean] = sql"""delete from "production"."productlistpricehistory" where "productid" = ${fromWrite(compositeId.productid)(using new Write.Single(ProductId.put))} AND "startdate" = ${fromWrite(compositeId.startdate)(using new Write.Single(TypoLocalDateTime.put))}""".update.run.map(_ > 0)
