...
class DocumentRepoImpl extends DocumentRepo {
  ...
  override def deleteByIds(documentnodes: Array[DocumentId]): ConnectionIO[Int] = {
    sql"""delete from "production"."document" where "documentnode" = ANY(${fromWrite(documentnodes)(using new Write.Single(DocumentId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(documentnodes: Array[DocumentId]): Stream[ConnectionIO, DocumentRow] = {
    sql"""select "title", "owner", "folderflag", "filename", "fileextension", "revision", "changenumber", "status", "documentsummary", "document", "rowguid", "modifieddate"::text, "documentnode" from "production"."document" where "documentnode" = ANY(${fromWrite(documentnodes)(using new Write.Single(DocumentId.arrayPut))})""".query(using DocumentRow.read).stream
  }
