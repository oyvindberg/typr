...
class ProductcategoryRepoImpl extends ProductcategoryRepo {
  ...
  def deleteById(productcategoryid: ProductcategoryId): ConnectionIO[Boolean] = sql"""delete from "production"."productcategory" where "productcategoryid" = ${fromWrite(productcategoryid)(using new Write.Single(ProductcategoryId.put))}""".update.run.map(_ > 0)

  def deleteByIds(productcategoryids: Array[ProductcategoryId]): ConnectionIO[Int] = sql"""delete from "production"."productcategory" where "productcategoryid" = ANY(${fromWrite(productcategoryids)(using new Write.Single(ProductcategoryId.arrayPut))})""".update.run
