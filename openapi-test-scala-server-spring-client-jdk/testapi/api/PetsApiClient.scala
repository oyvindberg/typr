...
class PetsApiClient(
  ...
  override def listPets(
    ...
    var request = java.net.http.HttpRequest.newBuilder(java.net.URI.create(baseUri.toString() + "/" + "pets" + (if (limit.isDefined) "?limit=" + limit.get.toString() else "") + (if (status.isDefined) "&status=" + status.get.toString() else ""))).method("GET", java.net.http.HttpRequest.BodyPublishers.noBody()).header("Content-Type", "application/json").header("Accept", "application/json").build()
    var response = httpClient.send(request, java.net.http.HttpResponse.BodyHandlers.ofString())
    return objectMapper.readValue(response.body(), new com.fasterxml.jackson.core.`type`.TypeReference[scala.List[testapi.model.Pet]] {})
  }
