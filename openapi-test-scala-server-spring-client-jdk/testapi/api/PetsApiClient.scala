...
class PetsApiClient(
  ...
  override def createPet(body: PetCreate): Response201400[Pet, Error] = {
    var request = java.net.http.HttpRequest
      ...
      .method("POST", java.net.http.HttpRequest.BodyPublishers.ofString(objectMapper.writeValueAsString(body)))
  ...
  override def deletePet(
    ...
    var request = java.net.http.HttpRequest
      .newBuilder(java.net.URI.create(baseUri.toString() + "/" + "pets" + "/" + petId.toString()))
  ...
  override def getPetPhoto(
    ...
    var request = java.net.http.HttpRequest
      .newBuilder(java.net.URI.create(baseUri.toString() + "/" + "pets" + "/" + petId.toString() + "/" + "photo"))
      .method("GET", java.net.http.HttpRequest.BodyPublishers.noBody())
      .header("Content-Type", "application/json")
      .header("Accept", "application/json")
      .build()
  ...
  override def listPets(
    ...
    var request = java.net.http.HttpRequest
      .newBuilder(
        java.net.URI.create(
          baseUri.toString() + "/" + "pets" + (if (limit.isPresent()) "?limit=" + limit.get().toString() else "") + (if (status.isPresent()) "&status=" + status.get().toString() else "")
        )
      )
      .method("GET", java.net.http.HttpRequest.BodyPublishers.noBody())
      .header("Content-Type", "application/json")
