package testapi.api

import com.fasterxml.jackson.annotation.JsonProperty
import scala.Nothing
import testapi.model.Error

/** Response type for: 201, 400 */
trait Response201400[+T201, +T400] {
  @JsonProperty("status")
  def status: String
}

/** Response type for: 200, 404 */
trait Response200404[+T200, +T404] {
  @JsonProperty("status")
  def status: String
}

/** Response type for: 200, 4XX, 5XX */
trait Response2004XX5XX[+T200] {
  @JsonProperty("status")
  def status: String
}

/** HTTP 400 response */
case class BadRequest[+T](@JsonProperty("value") value: T) extends Response201400[Nothing, T] {
  override lazy val status: String = "400"
}

/** HTTP 201 response */
case class Created[+T](@JsonProperty("value") value: T) extends Response201400[T, Nothing] {
  override lazy val status: String = "201"
}

/** HTTP 5XX response */
case class ServerError5XX(
    /** HTTP status code */
    @JsonProperty("statusCode") statusCode: Integer,
    @JsonProperty("value") value: Error
) extends Response2004XX5XX[Nothing] {
  override lazy val status: String = "5XX"
}

/** HTTP 200 response */
case class Ok[+T](@JsonProperty("value") value: T) extends Response200404[T, Nothing] with Response2004XX5XX[T] {
  override lazy val status: String = "200"
}

/** HTTP 404 response */
case class NotFound[+T](@JsonProperty("value") value: T) extends Response200404[Nothing, T] {
  override lazy val status: String = "404"
}

/** HTTP 4XX response */
case class ClientError4XX(
    /** HTTP status code */
    @JsonProperty("statusCode") statusCode: Integer,
    @JsonProperty("value") value: Error
) extends Response2004XX5XX[Nothing] {
  override lazy val status: String = "4XX"
}
