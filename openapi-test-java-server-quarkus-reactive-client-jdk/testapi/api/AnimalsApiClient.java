...
public class AnimalsApiClient implements AnimalsApi {
  ...
  @Override
  public Uni<Response2004XX5XX<List<Animal>>> listAnimals() {
    var request =
                ...
                if (statusCode == 200) {
                  return new Ok(
                      objectMapper.readValue(
                          response.body(), new TypeReference<List<Animal>>() {}));
                } else if (statusCode >= 400 && statusCode < 500) {
                  return new ClientError4XX(
                      statusCode, objectMapper.readValue(response.body(), Error.class));
                } else if (statusCode >= 500 && statusCode < 600) {
                  return new ServerError5XX(
                      statusCode, objectMapper.readValue(response.body(), Error.class));
                } else {
