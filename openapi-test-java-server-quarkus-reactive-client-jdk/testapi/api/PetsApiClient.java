...
public class PetsApiClient implements PetsApi {
  ...
  public Uni<Response201400<Pet, Error>> createPet(PetCreate body) {
    try {
      ...
      return Uni.createFrom().completionStage(() -> httpClient.sendAsync(request, BodyHandlers.ofString())).map(response -> {
        try {
  ...
  public Uni<List<Pet>> listPets(
    ...
    var request = HttpRequest.newBuilder(URI.create(baseUri.toString() + "/" + "pets" + (limit.isPresent()
      ? "?limit=" + limit.get().toString()
      : "") + (status.isPresent()
      ? "&status=" + status.get().toString()
      : ""))).method("GET", BodyPublishers.noBody()).header("Content-Type", "application/json").header("Accept", "application/json").build();
    return Uni.createFrom().completionStage(() -> httpClient.sendAsync(request, BodyHandlers.ofString())).map(response -> {
      try {
