...
/**
 * Integration tests for the OpenAPI generated Java Quarkus reactive server and JDK HTTP client code.
...
public class OpenApiIntegrationTest {
     ...
     * This allows testing the reactive client against a standard Jersey/Grizzly server.
     */
    @Path("/pets")
    public static class TestPetsApiServer {
        ...
        public Response getPet(@PathParam("petId") PetId petId) {
            ...
            if (pet != null) {
                ...
                return Response.status(404).entity(new testapi.model.Error("NOT_FOUND", Optional.empty(), "Pet not found")).build();
            }
        }

        @GET
        @Produces(MediaType.APPLICATION_JSON)
        public List<Pet> listPets(@QueryParam("limit") Optional<Integer> limit, @QueryParam("status") Optional<String> status) {
