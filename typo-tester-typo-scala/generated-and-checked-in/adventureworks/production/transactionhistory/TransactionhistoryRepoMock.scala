...
case class TransactionhistoryRepoMock(
  ...
  def deleteById(transactionid: TransactionhistoryId)(using c: Connection): java.lang.Boolean = Optional.ofNullable(map.remove(transactionid)).isPresent()
  ...
  def insert(unsaved: TransactionhistoryRowUnsaved)(using c: Connection): TransactionhistoryRow = insert(toRow(unsaved))(using c)
  ...
  def select: SelectBuilder[TransactionhistoryFields, TransactionhistoryRow] = new SelectBuilderMock(TransactionhistoryFields.structure, () => new ArrayList(map.values()), SelectParams.empty())

  def selectAll(using c: Connection): java.util.List[TransactionhistoryRow] = new ArrayList(map.values())

  def selectById(transactionid: TransactionhistoryId)(using c: Connection): Optional[TransactionhistoryRow] = Optional.ofNullable(map.get(transactionid))
  ...
  def selectByIdsTracked(transactionids: Array[TransactionhistoryId])(using c: Connection): java.util.Map[TransactionhistoryId, TransactionhistoryRow] = selectByIds(transactionids)(using c).stream().collect(Collectors.toMap((row: adventureworks.production.transactionhistory.TransactionhistoryRow) => row.transactionid, Function.identity()))
