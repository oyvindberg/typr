...
object TransactionhistoryRowUnsaved {
  given pgText: PgText[TransactionhistoryRowUnsaved] = {
    PgText.instance((row, sb) => {
      ...
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using TransactionhistoryId.pgType.pgText).unsafeEncode(row.transactionid, sb);
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using PgTypes.int4.pgText).unsafeEncode(row.referenceorderlineid, sb);
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using TypoLocalDateTime.pgType.pgText).unsafeEncode(row.transactiondate, sb);
      sb.append(PgText.DELIMETER);
