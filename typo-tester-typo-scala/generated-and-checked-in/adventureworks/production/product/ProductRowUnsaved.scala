...
object ProductRowUnsaved {
  given pgText: PgText[ProductRowUnsaved] = {
    PgText.instance((row, sb) => {
      ...
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using ProductId.pgType.pgText).unsafeEncode(row.productid, sb);
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using Flag.pgType.pgText).unsafeEncode(row.makeflag, sb);
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using Flag.pgType.pgText).unsafeEncode(row.finishedgoodsflag, sb);
      sb.append(PgText.DELIMETER);
