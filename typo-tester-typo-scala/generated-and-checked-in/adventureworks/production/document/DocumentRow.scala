...
object DocumentRow {
  val `_rowParser`: RowParser[DocumentRow] = {
    RowParsers.of(PgTypes.text, BusinessentityId.pgType, Flag.pgType, PgTypes.text, PgTypes.text.opt(), PgTypes.text, PgTypes.int4, TypoShort.pgType, PgTypes.text.opt(), TypoBytea.pgType.opt(), TypoUUID.pgType, TypoLocalDateTime.pgType, DocumentId.pgType, DocumentRow.apply, row => new Tuple13(
      row.title,
      row.owner,
      row.folderflag,
      row.filename,
