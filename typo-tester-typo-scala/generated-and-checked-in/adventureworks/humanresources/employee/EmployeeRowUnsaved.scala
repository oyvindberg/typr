...
object EmployeeRowUnsaved {
  given pgText: PgText[EmployeeRowUnsaved] = {
    PgText.instance((row, sb) => {
      ...
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using Flag.pgType.pgText).unsafeEncode(row.salariedflag, sb);
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using TypoShort.pgType.pgText).unsafeEncode(row.vacationhours, sb);
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using TypoShort.pgType.pgText).unsafeEncode(row.sickleavehours, sb);
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using Flag.pgType.pgText).unsafeEncode(row.currentflag, sb);
      sb.append(PgText.DELIMETER);
      ...
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using PgTypes.text.opt().pgText).unsafeEncode(row.organizationnode, sb);
    })
  }
}