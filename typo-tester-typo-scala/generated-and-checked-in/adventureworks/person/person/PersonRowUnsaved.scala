...
object PersonRowUnsaved {
  given pgText: PgText[PersonRowUnsaved] = {
    PgText.instance((row, sb) => {
      ...
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using NameStyle.pgType.pgText).unsafeEncode(row.namestyle, sb);
      sb.append(PgText.DELIMETER);
      Defaulted.pgText(using PgTypes.int4.pgText).unsafeEncode(row.emailpromotion, sb);
      sb.append(PgText.DELIMETER);
