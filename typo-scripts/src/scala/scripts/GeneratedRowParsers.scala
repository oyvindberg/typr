    ...
    targets.foreach { target =>
      target.project.value match {
        case "typo-runtime-java" =>
          ...
          FileUtils.writeString(started.logger, Some("writing"), target.sources.resolve("typo/runtime/RowParsers.java"), javaContents)

        case "typo-dsl-kotlin" =>
          ...
          val kotlinContents =
            s"""|package typo.kotlindsl
                |
                |import typo.runtime.DbType
                |import typo.runtime.RowParser as JavaRowParser
                |
                |/**
          ...
          FileUtils.writeString(started.logger, Some("writing"), target.sources.resolve("typo/kotlindsl/RowParsers.kt"), kotlinContents)
        ...
        case "typo-dsl-scala" =>
          ...
          val scalaConstructorMethods = 1
            ...
            .map { n =>
              ...
              s"""|  inline def of[$tparamsDecl, Row]($params)(decode: ($decodeParams) => Row)(encode: Row => Array[Any]): typo.scaladsl.RowParser[Row] = {
                  |    val javaParser = new typo.runtime.RowParser(java.util.List.of($typeList), a => decode($arrayIndices), r => encode(r))
                  |    new typo.scaladsl.RowParser(javaParser)
                  |  }""".stripMargin
            }

          val scalaContents =
            s"""|package typo.scaladsl
                |
                |import typo.runtime.DbType
                |
          ...
          FileUtils.writeString(started.logger, Some("writing"), target.sources.resolve("typo/scaladsl/RowParsers.scala"), scalaContents)
