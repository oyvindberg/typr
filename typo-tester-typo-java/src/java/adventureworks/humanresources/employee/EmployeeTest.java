...
public class EmployeeTest {
    ...
    public void works() {
        WithConnection.run(c -> {
            ...
            var personRow = personRepo.insert(
                ...
                .withAdditionalcontactinfo(Optional.of(new TypoXml("<additionalcontactinfo/>"))),
                c
            );

            // Setup employee - use short ctor + withers for specific values
            var unsaved = new EmployeeRowUnsaved(
                personRow.businessentityid(),
                "9912312312",
                "loginid",
                "jobtitle",
                new TypoLocalDate(LocalDate.of(1950, 1, 1)),
                "M",
                "F",
                new TypoLocalDate(LocalDate.now().minusYears(1))
            )
            .withSalariedflag(new Defaulted.Provided<>(new Flag(true)))
            .withVacationhours(new Defaulted.Provided<>(new TypoShort((short) 1)))
            .withSickleavehours(new Defaulted.Provided<>(new TypoShort((short) 2)))
            ...
            // Select by ids
            var byIds = employeeRepo.selectByIds(new BusinessentityId[]{saved1.businessentityid(), new BusinessentityId(22)}, c);
            ...
            var minimalUnsaved = new EmployeeRowUnsaved(
                personRow.businessentityid(),
                "9912312313",
                "loginid2",
                "jobtitle2",
                new TypoLocalDate(LocalDate.of(1960, 1, 1)),
                "M",
                "F",
                new TypoLocalDate(LocalDate.now().minusYears(1))
            );
            ...
            assertEquals(new TypoShort((short) 0), withDefaults.vacationhours());
            assertEquals(new TypoShort((short) 0), withDefaults.sickleavehours());
