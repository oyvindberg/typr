...
public class CompositeIdsTest extends SnapshotTest {
    ...
    private final ProductsubcategoryRepoImpl productsubcategoryRepo = new ProductsubcategoryRepoImpl();
    ...
    public void works() {
        WithConnection.run(c -> {
            ...
            var unitmeasure = unitmeasureRepo.insert(
                    new UnitmeasureRow(new UnitmeasureId("kgg"), new Name("Kilograms"), TypoLocalDateTime.now()),
                    c
            );
            ...
            var productSubcategory = productsubcategoryRepo.insert(
                    new ProductsubcategoryRowUnsaved(productCategory.productcategoryid(), new Name("Test Subcategory")), c
            );

            // Setup product model - use short ctor
            var productModel = productmodelRepo.insert(
                    new ProductmodelRowUnsaved(new Name("Test Model")), c
            );

            var now = TypoLocalDateTime.now();

            // Setup product - use short ctor + withers
            var product = productRepo.insert(
            ...
            var ph1 = repo.insert(
                    new ProductcosthistoryRowUnsaved(product.productid(), now, BigDecimal.ONE)
                            .withEnddate(Optional.of(TypoLocalDateTime.apply(now.value().plusDays(1)))), c
            );
            var ph2 = repo.insert(
                    new ProductcosthistoryRowUnsaved(product.productid(), TypoLocalDateTime.apply(now.value().plusDays(1)), BigDecimal.ONE)
                            .withEnddate(Optional.of(TypoLocalDateTime.apply(now.value().plusDays(2)))), c
            );
            var ph3 = repo.insert(
                    new ProductcosthistoryRowUnsaved(product.productid(), TypoLocalDateTime.apply(now.value().plusDays(2)), BigDecimal.ONE)
                            .withEnddate(Optional.of(TypoLocalDateTime.apply(now.value().plusDays(3)))), c
            );

            var wanted = new ProductcosthistoryId[]{
                    ph1.compositeId(),
                    ph2.compositeId(),
                    new ProductcosthistoryId(new ProductId(9999), ph3.compositeId().startdate())
            };

            var selected = repo.selectByIds(wanted, c).stream()
            ...
            var remaining = repo.selectAll(c).stream()
                    .map(ProductcosthistoryRow::compositeId)
                    .toList();
    ...
    private void testDsl(
        ...
        WithConnection.run(c -> {
            ...
            var businessentity1 = businessentityRepo.insert(
                    new BusinessentityRow(new BusinessentityId(1), TypoUUID.randomUUID(), new TypoLocalDateTime(now)), c
            );
            personRepo.insert(personRow(businessentity1.businessentityid(), 1, now), c);

            var businessentity2 = businessentityRepo.insert(
                    new BusinessentityRow(new BusinessentityId(2), TypoUUID.randomUUID(), new TypoLocalDateTime(now)), c
            );
            personRepo.insert(personRow(businessentity2.businessentityid(), 2, now), c);

            // Create email addresses for businessentity1
            var emailaddress1_1 = emailaddressRepo.insert(
                    new EmailaddressRow(businessentity1.businessentityid(), 1, Optional.of("a@b.c"), TypoUUID.randomUUID(), new TypoLocalDateTime(now)), c
            );
            var emailaddress1_2 = emailaddressRepo.insert(
                    new EmailaddressRow(businessentity1.businessentityid(), 2, Optional.of("aa@bb.cc"), TypoUUID.randomUUID(), new TypoLocalDateTime(now)), c
            );
            var emailaddress1_3 = emailaddressRepo.insert(
                    new EmailaddressRow(businessentity1.businessentityid(), 3, Optional.of("aaa@bbb.ccc"), TypoUUID.randomUUID(), new TypoLocalDateTime(now)), c
            );

            // Create email addresses for businessentity2
            emailaddressRepo.insert(
                    new EmailaddressRow(businessentity2.businessentityid(), 1, Optional.of("A@B.C"), TypoUUID.randomUUID(), new TypoLocalDateTime(now)), c
            );
            emailaddressRepo.insert(
                    new EmailaddressRow(businessentity2.businessentityid(), 2, Optional.of("AA@BB.CC"), TypoUUID.randomUUID(), new TypoLocalDateTime(now)), c
            );
            emailaddressRepo.insert(
                    new EmailaddressRow(businessentity2.businessentityid(), 3, Optional.of("AAA@BBB.CCC"), TypoUUID.randomUUID(), new TypoLocalDateTime(now)), c
            );

            // Test compositeIdIs
            var res1 = emailaddressRepo.select()
                    .where(e -> e.compositeIdIs(emailaddress1_1.compositeId()))
                    .toList(c);
            assertEquals(List.of(emailaddress1_1), res1);

            // Test compositeIdIn
            var query2 = emailaddressRepo.select()
                    .where(e -> e.compositeIdIn(List.of(emailaddress1_2.compositeId(), emailaddress1_3.compositeId())));
    ...
    public void dslInMemory() {
        testDsl(
                new EmailaddressRepoMock(unsaved -> unsaved.toRow(
                        () -> 0,  // emailaddressid
                        () -> TypoUUID.randomUUID(),
                        () -> TypoLocalDateTime.now()
                )),
                new BusinessentityRepoMock(unsaved -> unsaved.toRow(
                        () -> new BusinessentityId(0),  // businessentityid
                        () -> TypoUUID.randomUUID(),
                        () -> TypoLocalDateTime.now()
                )),
                new PersonRepoMock(unsaved -> unsaved.toRow(
