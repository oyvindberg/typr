...
public class SeekDbTest {
    ...
    private static final PgType<TypoLocalDateTime> timestampPgType = TypoLocalDateTime.pgType.withTypename("timestamp");
    ...
    private void testUniformSeek(BusinessentityRepo businessentityRepo) {
        ...
        for (int i = 0; i < limit * 2; i++) {
            ...
            var time = new TypoLocalDateTime(now.minusDays(i % limit));
            rows.add(new BusinessentityRow(new BusinessentityId(i), TypoUUID.randomUUID(), time));
        }
        ...
        sortedRows.sort(Comparator
                .comparing((BusinessentityRow r) -> r.modifieddate().value())
        ...
        WithConnection.run(c -> {
            ...
            var rows1 = businessentityRepo.select()
                    .maybeSeek(f -> f.modifieddate().asc(), Optional.empty(), v -> new SqlExpr.ConstReq<>(v, timestampPgType))
                    .maybeSeek(f -> f.businessentityid().asc(), Optional.empty(), v -> new SqlExpr.ConstReq<>(v, BusinessentityId.pgType))
                    .maybeSeek(f -> f.rowguid().asc(), Optional.empty(), v -> new SqlExpr.ConstReq<>(v, TypoUUID.pgType))
                    .limit(limit)
                    .toList(c);
            ...
            var rows2 = businessentityRepo.select()
                    .maybeSeek(f -> f.modifieddate().asc(), Optional.of(lastRow.modifieddate()), v -> new SqlExpr.ConstReq<>(v, timestampPgType))
                    .maybeSeek(f -> f.businessentityid().asc(), Optional.of(lastRow.businessentityid()), v -> new SqlExpr.ConstReq<>(v, BusinessentityId.pgType))
                    .maybeSeek(f -> f.rowguid().asc(), Optional.of(lastRow.rowguid()), v -> new SqlExpr.ConstReq<>(v, TypoUUID.pgType))
                    .limit(limit)
                    .toList(c);
    ...
    public void uniformInMemory() {
        ...
        testUniformSeek(new BusinessentityRepoMock(unsaved -> { throw new UnsupportedOperationException(); }));
    }
    ...
    private void testNonUniformSeek(BusinessentityRepo businessentityRepo) {
        ...
        List<BusinessentityRow> sortedRows = new ArrayList<>(rows);
        sortedRows.sort(Comparator
                .comparing((BusinessentityRow r) -> r.modifieddate().value()).reversed()
                .thenComparing(r -> r.businessentityid().value()));
        ...
        WithConnection.run(c -> {
            ...
            var rows1 = businessentityRepo.select()
                    .maybeSeek(f -> f.modifieddate().desc(), Optional.empty(), v -> new SqlExpr.ConstReq<>(v, timestampPgType))
                    .maybeSeek(f -> f.businessentityid().asc(), Optional.empty(), v -> new SqlExpr.ConstReq<>(v, BusinessentityId.pgType))
                    .maybeSeek(f -> f.rowguid().asc(), Optional.empty(), v -> new SqlExpr.ConstReq<>(v, TypoUUID.pgType))
                    .limit(limit)
                    .toList(c);
            ...
            var lastRow = rows1.get(rows1.size() - 1);
            var rows2 = businessentityRepo.select()
                    .maybeSeek(f -> f.modifieddate().desc(), Optional.of(lastRow.modifieddate()), v -> new SqlExpr.ConstReq<>(v, timestampPgType))
                    .maybeSeek(f -> f.businessentityid().asc(), Optional.of(lastRow.businessentityid()), v -> new SqlExpr.ConstReq<>(v, BusinessentityId.pgType))
                    .maybeSeek(f -> f.rowguid().asc(), Optional.of(lastRow.rowguid()), v -> new SqlExpr.ConstReq<>(v, TypoUUID.pgType))
                    .limit(limit)
                    .toList(c);
    ...
    public void nonUniformInMemory() {
        // The mock's toRow converter isn't used because we insert fully-constructed rows
        testNonUniformSeek(new BusinessentityRepoMock(unsaved -> { throw new UnsupportedOperationException(); }));
    }
