 ...
 * Equivalent to Scala OpenEnumTest.
 */
public class OpenEnumTest {
    ...
    public void works() {
        WithConnection.run(c -> {
            ...
            var john = testInsert.publicTitledperson(TitleDomainId.Known.dr, TitleId.Known.dr, "John", c);

            // DSL query with joinFk chain
            var found = titledPersonRepo.select()
    ...
    private void testDsl(
        ...
        WithConnection.run(c -> {
            ...
            var john = titledPersonRepo.insert(
                    new TitledpersonRow(TitleDomainId.Known.dr, TitleId.Known.dr, "John"),
                    c
            );
            ...
            String titleString = switch (titleValue) {
