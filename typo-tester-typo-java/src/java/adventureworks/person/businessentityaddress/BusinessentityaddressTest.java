...
public class BusinessentityaddressTest {
    private final BusinessentityaddressRepoImpl businessentityaddressRepo = new BusinessentityaddressRepoImpl();
    ...
    public void works() {
        WithConnection.run(c -> {
            ...
            var businessentityRow = businessentityRepo.insert(
            ...
            var countryregion = countryregionRepo.insert(
                    new CountryregionRowUnsaved(new CountryregionId("max"), new Name("max")), c
            );

            var salesTerritory = salesterritoryRepo.insert(
                    new SalesterritoryRowUnsaved(new Name("name"), countryregion.countryregioncode(), "flaff")
                            .withSalesytd(new Defaulted.Provided<>(java.math.BigDecimal.ONE)), c
            );

            var stateProvidence = stateprovinceRepo.insert(
                    new StateprovinceRowUnsaved("cde", countryregion.countryregioncode(), new Name("name"), salesTerritory.territoryid()), c
            );

            var address = addressRepo.insert(
                    new AddressRowUnsaved("addressline1", "city", stateProvidence.stateprovinceid(), "postalcode")
                            .withAddressline2(Optional.of("addressline2")), c
            );

            var addressType = addresstypeRepo.insert(
                    new AddresstypeRowUnsaved(new Name("name")), c
            );

            var unsaved1 = new BusinessentityaddressRowUnsaved(
            ...
            // check field values
            var newModifiedDate = TypoLocalDateTime.apply(saved1.modifieddate().value().minusDays(1));
