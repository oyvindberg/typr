    ...
    List<BusinessentityaddressRow> syncAddresses(PersonWithAddresses pa, Connection c) {
        ...
        pa.addresses().forEach((addressTypeName, wanted) -> {
            ...
            if (addresstypeId != null) {
                ...
                var inserted = addresstypeRepo.insert(new AddresstypeRowUnsaved(addressTypeName), c);
        ...
        businessentityAddressRepo.select()
                ...
                .toList(c)
                .forEach(x -> oldAttachedAddresses.put(new AddressIdAddresstypeIdKey(x.addressid(), x.addresstypeid()), x));

        // unattach old attached addresses
        oldAttachedAddresses.forEach((key, ba) -> {
        ...
        currentAddressesByType.forEach((addresstypeId, address) -> {
            ...
            if (existing != null) {
                ...
                var newRow = new BusinessentityaddressRowUnsaved(pa.person().businessentityid(), address.addressid(), addresstypeId);
    ...
    public void works() {
        WithConnection.run(c -> {
            ...
            var countryregionRow = testInsert.personCountryregion(
                    new CountryregionId("NOR"),
                    new Name("Norway"),
                    new Defaulted.UseDefault<>(),
                    c
            );
            var salesterritoryRow = testInsert.salesSalesterritory(
            ...
            var stateprovinceRow = testInsert.personStateprovince(
                    countryregionRow.countryregioncode(),
            ...
            var addressRow1 = testInsert.personAddress(
            ...
            var addressRow2 = testInsert.personAddress(
                    stateprovinceRow.stateprovinceid(),
                    "Street 2",
                    Optional.empty(),
                    "Oslo",
            ...
            var addressRow3 = testInsert.personAddress(
                    stateprovinceRow.stateprovinceid(),
                    "Street 3",
                    Optional.empty(),
                    "Oslo",
            ...
            repo.syncAddresses(new PersonWithAddresses(personRow, Map.of(
                    new Name("HOME"), addressRow1,
                    new Name("OFFICE"), addressRow2
            )), c);

            // Helper to fetch business entity addresses
            var allAddressIds = new AddressId[]{addressRow1.addressid(), addressRow2.addressid(), addressRow3.addressid()};
            var fetchBAs = (java.util.function.Supplier<List<BusinessentityaddressRow>>) () ->
                    businessentityaddressRepo.select()
