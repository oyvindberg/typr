/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package adventureworks.purchasing.shipmethod;

import adventureworks.customtypes.Defaulted;
import adventureworks.public_.Name;
import java.math.BigDecimal;
import java.time.LocalDateTime;
import java.util.UUID;
import typo.runtime.PgText;
import typo.runtime.PgTypes;
import typo.runtime.RowParser;
import typo.runtime.RowParsers;

/** Table: purchasing.shipmethod
  * Shipping company lookup table.
  * Primary key: shipmethodid
  */
public record ShipmethodRow(
  /** Primary key for ShipMethod records.
    * Default: nextval('purchasing.shipmethod_shipmethodid_seq'::regclass)
    */
  ShipmethodId shipmethodid,
  /** Shipping company name. */
  Name name,
  /** Minimum shipping charge.
    * Default: 0.00
    * Constraint CK_ShipMethod_ShipBase affecting columns shipbase: ((shipbase > 0.00))
    */
  BigDecimal shipbase,
  /** Shipping charge per pound.
    * Default: 0.00
    * Constraint CK_ShipMethod_ShipRate affecting columns shiprate: ((shiprate > 0.00))
    */
  BigDecimal shiprate,
  /** Default: uuid_generate_v1() */
  UUID rowguid,
  /** Default: now() */
  LocalDateTime modifieddate
) {
  /** Primary key for ShipMethod records.
    * Default: nextval('purchasing.shipmethod_shipmethodid_seq'::regclass)
    */
  public ShipmethodRow withShipmethodid(ShipmethodId shipmethodid) {
    return new ShipmethodRow(shipmethodid, name, shipbase, shiprate, rowguid, modifieddate);
  };

  /** Shipping company name. */
  public ShipmethodRow withName(Name name) {
    return new ShipmethodRow(shipmethodid, name, shipbase, shiprate, rowguid, modifieddate);
  };

  /** Minimum shipping charge.
    * Default: 0.00
    * Constraint CK_ShipMethod_ShipBase affecting columns shipbase: ((shipbase > 0.00))
    */
  public ShipmethodRow withShipbase(BigDecimal shipbase) {
    return new ShipmethodRow(shipmethodid, name, shipbase, shiprate, rowguid, modifieddate);
  };

  /** Shipping charge per pound.
    * Default: 0.00
    * Constraint CK_ShipMethod_ShipRate affecting columns shiprate: ((shiprate > 0.00))
    */
  public ShipmethodRow withShiprate(BigDecimal shiprate) {
    return new ShipmethodRow(shipmethodid, name, shipbase, shiprate, rowguid, modifieddate);
  };

  /** Default: uuid_generate_v1() */
  public ShipmethodRow withRowguid(UUID rowguid) {
    return new ShipmethodRow(shipmethodid, name, shipbase, shiprate, rowguid, modifieddate);
  };

  /** Default: now() */
  public ShipmethodRow withModifieddate(LocalDateTime modifieddate) {
    return new ShipmethodRow(shipmethodid, name, shipbase, shiprate, rowguid, modifieddate);
  };

  static RowParser<ShipmethodRow> _rowParser = RowParsers.of(ShipmethodId.pgType, Name.pgType, PgTypes.numeric, PgTypes.numeric, PgTypes.uuid, PgTypes.timestamp, (t0, t1, t2, t3, t4, t5) -> new ShipmethodRow(t0, t1, t2, t3, t4, t5), row -> new Object[]{row.shipmethodid(), row.name(), row.shipbase(), row.shiprate(), row.rowguid(), row.modifieddate()});;

  static public PgText<ShipmethodRow> pgText =
    PgText.from(_rowParser);

  public ShipmethodId id() {
    return shipmethodid;
  };

  public ShipmethodRowUnsaved toUnsavedRow(
    Defaulted<ShipmethodId> shipmethodid,
    Defaulted<BigDecimal> shipbase,
    Defaulted<BigDecimal> shiprate,
    Defaulted<UUID> rowguid,
    Defaulted<LocalDateTime> modifieddate
  ) {
    return new ShipmethodRowUnsaved(name, shipmethodid, shipbase, shiprate, rowguid, modifieddate);
  };
}