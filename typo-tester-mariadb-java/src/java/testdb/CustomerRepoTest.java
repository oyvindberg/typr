...
/**
 * Full repository test for customers - tests all CRUD operations and more. Note: The database has
  ...
  public void testSelectById() {
    WithConnection.run(
        c -> {
          ...
          var unsaved =
              new CustomersRowUnsaved("test@example.com", "hash".getBytes(), "Test", "User");
          ...
          var customer3 =
              customersRepo.insert(
                  new CustomersRowUnsaved(
                      "c3@example.com", "hash3".getBytes(), "Customer", "Three"),
                  c);
    ...
    WithConnection.run(
        c -> {
          ...
          var inserted =
              customersRepo.insert(
                  new CustomersRowUnsaved(
                      "unique@example.com", "hash".getBytes(), "Unique", "User"),
                  c);
  ...
  public void testInsertRow() {
    WithConnection.run(
        c -> {
          ...
          var unsaved =
              new CustomersRowUnsaved(
                  "insert@example.com", "password".getBytes(), "Insert", "Test");
          var inserted = customersRepo.insert(unsaved, c);
    ...
    WithConnection.run(
        c -> {
          ...
          var inserted =
              customersRepo.insert(
                  new CustomersRowUnsaved(
                      "update@example.com", "hash".getBytes(), "Before", "Update"),
                  c);

          var updated = inserted.withFirstName("After").withLastName("Changed").withTier("silver");
    ...
    WithConnection.run(
        c -> {
          ...
          var initial =
              customersRepo.insert(
                  new CustomersRowUnsaved(
                      "upsert@example.com", "hash".getBytes(), "Upsert", "Test"),
                  c);
    ...
    WithConnection.run(
        c -> {
          ...
          var inserted =
              customersRepo.insert(
                  new CustomersRowUnsaved(
                      "upsert.update@example.com", "hash".getBytes(), "Before", "Upsert"),
                  c);

          var modified = inserted.withFirstName("After").withTier("gold");
  ...
  public void testDeleteByIds() {
    WithConnection.run(
        c -> {
          ...
          var c1 =
              customersRepo.insert(
                  new CustomersRowUnsaved("del1@example.com", "hash1".getBytes(), "Del", "One"), c);
          var c2 =
              customersRepo.insert(
                  new CustomersRowUnsaved("del2@example.com", "hash2".getBytes(), "Del", "Two"), c);
          var c3 =
              customersRepo.insert(
  ...
  public void testDSLSelect() {
    WithConnection.run(
        c -> {
          // Use existing seeded 'pending' status (default)
          customersRepo.insert(
              new CustomersRowUnsaved("dsl1@example.com", "hash1".getBytes(), "Alice", "Smith"), c);
          customersRepo.insert(
              new CustomersRowUnsaved("dsl2@example.com", "hash2".getBytes(), "Bob", "Jones"), c);
          customersRepo.insert(
              new CustomersRowUnsaved("dsl3@example.com", "hash3".getBytes(), "Alice", "Brown"), c);

          // Test select with where clause
          var alices = customersRepo.select().where(f -> f.firstName().isEqual("Alice")).toList(c);
    ...
    WithConnection.run(
        c -> {
          ...
          var customer =
              customersRepo.insert(
                  new CustomersRowUnsaved(
                      "dsl.update@example.com", "hash".getBytes(), "Before", "Update"),
                  c);
  ...
  public void testDSLDelete() {
    WithConnection.run(
        c -> {
          ...
          customersRepo.insert(
              new CustomersRowUnsaved(
                  "dsl.del1@example.com", "hash1".getBytes(), "ToDelete", "One"),
              c);
          customersRepo.insert(
              new CustomersRowUnsaved(
                  "dsl.del2@example.com", "hash2".getBytes(), "ToDelete", "Two"),
              c);
          customersRepo.insert(
              new CustomersRowUnsaved("dsl.keep@example.com", "hash3".getBytes(), "ToKeep", "One"),
              c);

          customersRepo.delete().where(f -> f.firstName().isEqual("ToDelete")).execute(c);
