...
/**
 * Kotlin wrapper for typo.runtime.RowParser that provides Kotlin-native methods.
 *
...
class RowParser<Row>(val underlying: typo.runtime.RowParser<Row>) {
    ...
    fun all(): ResultSetParser<List<Row>> {
        ...
        return ResultSetParser(typo.runtime.ResultSetParser { rs -> javaParser.apply(rs).toList() })
    }
    ...
    fun first(): ResultSetParser<Row?> {
        val javaParser = typo.runtime.ResultSetParser.First(underlying)
        return ResultSetParser(typo.runtime.ResultSetParser { rs -> javaParser.apply(rs).orNull() })
    }
    ...
    fun maxOne(): ResultSetParser<Row?> {
        val javaParser = typo.runtime.ResultSetParser.MaxOne(underlying)
        return ResultSetParser(typo.runtime.ResultSetParser { rs -> javaParser.apply(rs).orNull() })
    }
