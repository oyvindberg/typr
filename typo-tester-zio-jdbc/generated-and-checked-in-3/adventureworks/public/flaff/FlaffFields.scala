...
trait FlaffFields {
  ...
  def compositeIdIn(compositeIds: Array[FlaffId]): SqlExpr[Boolean, Required] =
    new CompositeIn(compositeIds)(TuplePart[FlaffId](code)(_.code)(using as[Array[ShortText], Required](using ShortText.arrayJdbcEncoder, PGType.forArray(using ShortText.pgType)), implicitly), TuplePart[FlaffId](anotherCode)(_.anotherCode)(using as[Array[/* max 20 chars */ String], Required](using adventureworks.StringArrayEncoder, PGType.forArray(using PGType.PGTypeString)), implicitly), TuplePart[FlaffId](someNumber)(_.someNumber)(using as[Array[Int], Required](using adventureworks.IntArrayEncoder, PGType.forArray(using PGType.PGTypeInt)), implicitly), TuplePart[FlaffId](specifier)(_.specifier)(using as[Array[ShortText], Required](using ShortText.arrayJdbcEncoder, PGType.forArray(using ShortText.pgType)), implicitly))
