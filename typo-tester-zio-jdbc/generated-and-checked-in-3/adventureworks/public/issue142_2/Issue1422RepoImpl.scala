...
class Issue1422RepoImpl extends Issue1422Repo {
  ...
  def deleteById(tabellkode: Issue142Id): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from "public"."issue142_2" where "tabellkode" = ${Segment.paramSegment(tabellkode)(using Issue142Id.setter)}""".delete.map(_ > 0)

  def deleteByIds(tabellkodes: Array[Issue142Id]): ZIO[ZConnection, Throwable, Long] = sql"""delete from "public"."issue142_2" where "tabellkode" = ANY(${Segment.paramSegment(tabellkodes)(using Issue142Id.arraySetter)})""".delete
  ...
  def selectAll: ZStream[ZConnection, Throwable, Issue1422Row] = sql"""select "tabellkode" from "public"."issue142_2"""".query(using Issue1422Row.jdbcDecoder).selectStream()

  def selectById(tabellkode: Issue142Id): ZIO[ZConnection, Throwable, Option[Issue1422Row]] = sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ${Segment.paramSegment(tabellkode)(using Issue142Id.setter)}""".query(using Issue1422Row.jdbcDecoder).selectOne

  def selectByIds(tabellkodes: Array[Issue142Id]): ZStream[ZConnection, Throwable, Issue1422Row] = sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ANY(${Segment.paramSegment(tabellkodes)(using Issue142Id.arraySetter)})""".query(using Issue1422Row.jdbcDecoder).selectStream()
