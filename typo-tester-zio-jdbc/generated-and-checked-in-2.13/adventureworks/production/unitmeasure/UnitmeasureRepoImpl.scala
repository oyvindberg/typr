...
class UnitmeasureRepoImpl extends UnitmeasureRepo {
  ...
  def selectAll: ZStream[ZConnection, Throwable, UnitmeasureRow] = sql"""select "unitmeasurecode", "name", "modifieddate"::text from "production"."unitmeasure"""".query(UnitmeasureRow.jdbcDecoder).selectStream()

  def selectById(unitmeasurecode: UnitmeasureId): ZIO[ZConnection, Throwable, Option[UnitmeasureRow]] = sql"""select "unitmeasurecode", "name", "modifieddate"::text from "production"."unitmeasure" where "unitmeasurecode" = ${Segment.paramSegment(unitmeasurecode)(UnitmeasureId.setter)}""".query(UnitmeasureRow.jdbcDecoder).selectOne
