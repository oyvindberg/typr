...
class CurrencyRepoImpl extends CurrencyRepo {
  ...
  def selectAll: ZStream[ZConnection, Throwable, CurrencyRow] = sql"""select "currencycode", "name", "modifieddate"::text from "sales"."currency"""".query(CurrencyRow.jdbcDecoder).selectStream()

  def selectById(currencycode: CurrencyId): ZIO[ZConnection, Throwable, Option[CurrencyRow]] = sql"""select "currencycode", "name", "modifieddate"::text from "sales"."currency" where "currencycode" = ${Segment.paramSegment(currencycode)(CurrencyId.setter)}""".query(CurrencyRow.jdbcDecoder).selectOne

  def selectByIds(currencycodes: Array[CurrencyId]): ZStream[ZConnection, Throwable, CurrencyRow] = sql"""select "currencycode", "name", "modifieddate"::text from "sales"."currency" where "currencycode" = ANY(${Segment.paramSegment(currencycodes)(CurrencyId.arraySetter)})""".query(CurrencyRow.jdbcDecoder).selectStream()
