...
class PmpdcViewRepoImpl extends PmpdcViewRepo {
  ...
  def selectAll: ZStream[ZConnection, Throwable, PmpdcViewRow] = sql"""select "productmodelid", "productdescriptionid", "cultureid", "modifieddate"::text from "pr"."pmpdc"""".query(PmpdcViewRow.jdbcDecoder).selectStream()
}