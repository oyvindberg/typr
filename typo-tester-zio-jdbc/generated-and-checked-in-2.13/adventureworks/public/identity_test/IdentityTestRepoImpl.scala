...
class IdentityTestRepoImpl extends IdentityTestRepo {
  ...
  def selectAll: ZStream[ZConnection, Throwable, IdentityTestRow] = sql"""select "always_generated", "default_generated", "name" from "public"."identity-test"""".query(IdentityTestRow.jdbcDecoder).selectStream()

  def selectById(name: IdentityTestId): ZIO[ZConnection, Throwable, Option[IdentityTestRow]] = sql"""select "always_generated", "default_generated", "name" from "public"."identity-test" where "name" = ${Segment.paramSegment(name)(IdentityTestId.setter)}""".query(IdentityTestRow.jdbcDecoder).selectOne

  def selectByIds(names: Array[IdentityTestId]): ZStream[ZConnection, Throwable, IdentityTestRow] = sql"""select "always_generated", "default_generated", "name" from "public"."identity-test" where "name" = ANY(${Segment.paramSegment(names)(IdentityTestId.arraySetter)})""".query(IdentityTestRow.jdbcDecoder).selectStream()
