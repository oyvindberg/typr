...
class OnlyPkColumnsRepoImpl extends OnlyPkColumnsRepo {
  ...
  def deleteById(compositeId: OnlyPkColumnsId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from "public"."only_pk_columns" where "key_column_1" = ${Segment.paramSegment(compositeId.keyColumn1)(Setter.stringSetter)} AND "key_column_2" = ${Segment.paramSegment(compositeId.keyColumn2)(Setter.intSetter)}""".delete.map(_ > 0)
  ...
  def selectAll: ZStream[ZConnection, Throwable, OnlyPkColumnsRow] = sql"""select "key_column_1", "key_column_2" from "public"."only_pk_columns"""".query(OnlyPkColumnsRow.jdbcDecoder).selectStream()
