...
class FootballClubRepoImpl extends FootballClubRepo {
  ...
  override def deleteByIds(ids: Array[FootballClubId]): ConnectionIO[Int] = {
    sql"""delete from "myschema"."football_club" where "id" = ANY(${fromWrite(ids)(new Write.Single(FootballClubId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(ids: Array[FootballClubId]): Stream[ConnectionIO, FootballClubRow] = {
    sql"""select "id", "name" from "myschema"."football_club" where "id" = ANY(${fromWrite(ids)(new Write.Single(FootballClubId.arrayPut))})""".query(FootballClubRow.read).stream
  }
