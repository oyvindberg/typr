...
class PersonRepoImpl extends PersonRepo {
  ...
  override def deleteByIds(ids: Array[PersonId]): ConnectionIO[Int] = {
    sql"""delete from "myschema"."person" where "id" = ANY(${fromWrite(ids)(new Write.Single(PersonId.arrayPut))})""".update.run
  }
  ...
  override def selectByIds(ids: Array[PersonId]): Stream[ConnectionIO, PersonRow] = {
    sql"""select "id", "favourite_football_club_id", "name", "nick_name", "blog_url", "email", "phone", "likes_pizza", "marital_status_id", "work_email", "sector", "favorite_number" from "myschema"."person" where "id" = ANY(${fromWrite(ids)(new Write.Single(PersonId.arrayPut))})""".query(PersonRow.read).stream
  }
