...
class PersonRepoImpl extends PersonRepo {
  def delete: DeleteBuilder[PersonFields, PersonRow] = DeleteBuilder.of(""""myschema"."person"""", PersonFields.structure, PersonRow.read)

  def deleteById(id: PersonId): ConnectionIO[Boolean] = sql"""delete from "myschema"."person" where "id" = ${fromWrite(id)(new Write.Single(PersonId.put))}""".update.run.map(_ > 0)

  def deleteByIds(ids: Array[PersonId]): ConnectionIO[Int] = sql"""delete from "myschema"."person" where "id" = ANY(${fromWrite(ids)(new Write.Single(PersonId.arrayPut))})""".update.run
  ...
  def select: SelectBuilder[PersonFields, PersonRow] = SelectBuilder.of(""""myschema"."person"""", PersonFields.structure, PersonRow.read)

  def selectAll: Stream[ConnectionIO, PersonRow] = sql"""select "id", "favourite_football_club_id", "name", "nick_name", "blog_url", "email", "phone", "likes_pizza", "marital_status_id", "work_email", "sector", "favorite_number" from "myschema"."person"""".query(PersonRow.read).stream
  ...
  def selectById(id: PersonId): ConnectionIO[Option[PersonRow]] = sql"""select "id", "favourite_football_club_id", "name", "nick_name", "blog_url", "email", "phone", "likes_pizza", "marital_status_id", "work_email", "sector", "favorite_number" from "myschema"."person" where "id" = ${fromWrite(id)(new Write.Single(PersonId.put))}""".query(PersonRow.read).option
  ...
  def update: UpdateBuilder[PersonFields, PersonRow] = UpdateBuilder.of(""""myschema"."person"""", PersonFields.structure, PersonRow.read)
