...
case class PersonRepoMock(
  ...
  def deleteById(compositeId: PersonId)(using c: Connection): Boolean = map.remove(compositeId).isDefined
  ...
  def selectById(compositeId: PersonId)(using c: Connection): Option[PersonRow] = map.get(compositeId)
